...
def _axis_limited_speed(
    ...
    *,
    ...
    limit_x = (
        float("inf") if max_tx <= _PLANNER_RADIUS_EPS else (float(v_x_max) / max_tx)
    )
    limit_y = (
        float("inf") if max_ty <= _PLANNER_RADIUS_EPS else (float(v_y_max) / max_ty)
    )
...
def _planner_radius_speed_from_segments(segments: list[MotionSegment]) -> float:
    arc_speeds = [
        float(seg.speed) for seg in segments if seg.seg_type == SEG_TYPE_CIRCLE
    ]
    if not arc_speeds:
        return 0.0
