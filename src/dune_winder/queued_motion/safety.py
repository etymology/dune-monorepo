...
def motion_safety_limits_from_calibration(calibration) -> MotionSafetyLimits:
    ...
    return MotionSafetyLimits(
        ...
        transfer_zone_head_min_x=calibration_geometry_float(
            "transferZoneHeadMinX", 400.0
        ),
        transfer_zone_head_max_x=calibration_geometry_float(
            "transferZoneHeadMaxX", 500.0
        ),
        transfer_zone_foot_min_x=calibration_geometry_float(
            "transferZoneFootMinX", 7100.0
        ),
        transfer_zone_foot_max_x=calibration_geometry_float(
            "transferZoneFootMaxX", 7200.0
        ),
...
def _z_collision_is_active(
    ...
    if collision_state is None:
        return False
    return float(collision_state.z_actual_position) > float(
        limits.queued_motion_z_collision_threshold
    )
...
def _point_in_headward_pivot_keepout(
    x: float, y: float, limits: MotionSafetyLimits
) -> bool:
    return (
...
def _validate_point(
    ...
    if (
        x < limits.limit_left - limits.geometry_epsilon
        or x > limits.limit_right + limits.geometry_epsilon
    ):
        raise ValueError(
    ...
    if (
        y < limits.limit_bottom - limits.geometry_epsilon
        or y > limits.limit_top + limits.geometry_epsilon
    ):
        raise ValueError(
    ...
    if (
        ...
        raise ValueError(
            f"{label} seq={seq} enters forbidden transfer region (X={x:.3f}, Y={y:.3f})"
        )
...
def _validate_arc_step(
    ...
    for step in range(1, steps + 1):
        ...
        _validate_line_step(
            prev_x, prev_y, x, y, limits, seg.seq, "arc", collision_state
        )
        prev_x = x
        prev_y = y
...
def validate_segments_within_safety_limits(
    ...
    for seg in segments:
        if seg.seg_type == SEG_TYPE_CIRCLE:
            _validate_arc_step(
                prev_x, prev_y, seg, limits, queued_motion_collision_state
            )
