...
def build_waypoint_circles(
    *,
    ...
    for index, point_xy in enumerate(waypoints[:-1]):
        ...
        circles.append(
            WaypointCircle(waypoint_xy=point_xy, center_xy=center_xy, radius=radius)
        )
        prev_xy = point_xy
...
def point_circle_tangent_points(
    ...
    for sign in (-1.0, 1.0):
        ...
        if not any(
            distance_xy(tangent_xy, existing) <= 1e-6 for existing in tangent_points
        ):
...
def arc_choice_through_waypoint(
    ...
    if (
        ...
        or distance_xy(outgoing_xy, circle.waypoint_xy)
    ...
    def tangent_at(
        point_xy: tuple[float, float], direction: int
    ) -> tuple[float, float]:
    ...
    for sweep, direction in candidates:
        ...
        start_dot = (
            start_tangent[0] * incoming_dir[0] + start_tangent[1] * incoming_dir[1]
        )
...
def filleted_polygon_segments(
    *,
    ...
    circles = build_waypoint_circles(
        start_xy=start_xy, waypoints=waypoints, radius=radius
    )
    ...
    for index in range(1, len(points) - 1):
        interior_angle = _interior_angle(
        ...
        if circle_index == len(circles) - 1:
            ...
            for outgoing_xy in end_tangent_options:
                ...
                score = (
                    ...
                    + distance_xy(outgoing_xy, end_xy),
                )
