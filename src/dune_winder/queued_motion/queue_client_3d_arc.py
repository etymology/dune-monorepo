...
class MotionArc3DQueueClient:
    ...
    def wait_until_idle(
        ...
        while time.monotonic() < deadline:
            ...
            active_seq = (
                int(active_raw) if isinstance(active_raw, (int, float)) else None
            )
            ...
            if motion_fault:
                raise RuntimeError(
                    "PLC MotionFault3D became true while waiting for idle"
                )
...
def run_arc3d_queue_case(
    ...
    while next_to_enqueue < total:
        ...
        if motion_fault:
            raise RuntimeError(
                "PLC MotionFault3D became true while streaming queued segments"
            )

        queue_fault = motion._try_read_one(TAG_QUEUE_FAULT)
        if queue_fault:
            raise RuntimeError(
                "PLC QueueFault3D became true while streaming queued segments"
            )
