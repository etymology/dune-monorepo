...
def _path_length(
    points: list[tuple[float, float]], start_xy: Optional[tuple[float, float]]
) -> float:
    if not points:
        return 0.0
    ...
    while remaining:
        if _planner_timed_out(planner_deadline):
            _raise_planner_timeout(
                planner_timeout_s, "building nearest-neighbor seed path"
            )
        next_idx = min(
            remaining,
...
def _two_opt_open_path(
    ...
    while improved:
        if _planner_timed_out(planner_deadline):
            _raise_planner_timeout(
                planner_timeout_s, "running 2-opt waypoint optimization"
            )
        improved = False
        n = len(out)
...
def lissajous_segments(
    ...
    if (
        len(points) >= 2
        and math.hypot(points[-1][0] - points[0][0], points[-1][1] - points[0][1])
        < 1e-6
    ):
        points.pop()
        t_values.pop()
...
def waypoint_path_segments(
    ...
    if filleted is not None:
        ...
        if waypoint_bounds is None or _segments_within_bounds(
            segments, waypoint_bounds
        ):
            return segments
...
def enforce_min_segment_length(
    ...
    if len(filtered) >= 2:
        ...
        if math.hypot(last.x - prev.x, last.y - prev.y) < min_segment_length:
            ...
            if (
                math.hypot(last.x - prev_prev.x, last.y - prev_prev.y)
                >= min_segment_length
            ):
                filtered[-1] = last
            else:
...
def _merge_is_tangential(
    ...
    prev_tangent = _segment_tangent_vector(
        prev_start_x, prev_start_y, prev_seg, at_end=True
    )
    next_tangent = _segment_tangent_vector(
        prev_seg.x, prev_seg.y, next_seg, at_end=False
    )
...
def tune_segments_for_constant_velocity(
    ...
    if has_circle_segments(segments):
        ...
        tuned = enforce_max_segment_length(
            tuned, effective_min_length * max_segment_factor
        )
...
def print_pattern_summary(
    ...
    vmax_from_curvature = (
        math.sqrt(accel / kmax) if (kmax > 0 and accel > 0) else float("inf")
    )
    vmax_text = (
        f"{vmax_from_curvature:.1f}" if math.isfinite(vmax_from_curvature) else "inf"
    )
    print(
