...
class RllParser:
    ...
    def _parse_branch(self, tokens, index: int):
        branches = []
        while True:
            branch_nodes, index = self._parse_nodes(
                tokens, index, stop_tokens=frozenset({"NXB", "BND"})
            )
    ...
    def _parse_instruction(self, tokens, index: int):
        ...
        if opcode == "CMP":
            operands, end = self._collect_formula_operands(
                tokens, index + 1, required_prefix=0
            )
            return InstructionCall(opcode=opcode, operands=operands), end
        if opcode == "CPT":
            operands, end = self._collect_formula_operands(
                tokens, index + 1, required_prefix=1
            )
            return InstructionCall(opcode=opcode, operands=operands), end
