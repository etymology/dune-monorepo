...
FORMULA_FUNCTIONS = {
    ...
    "MOD": lambda left, right: (
        math.fmod(float(left), float(right)) if float(right) != 0 else 0.0
    ),
}
...
class ScanContext:
    ...
    def get_value(self, path: str, program: str | None = None):
        ...
        try:
            ...
            unique_program = (
                self._unique_program_for_path(path) if target_program is None else None
            )
    ...
    def exists(self, path: str, program: str | None = None) -> bool:
        ...
        if unique_program is not None and self.tag_store.exists(
            path, program=unique_program
        ):
            return True
...
class ExpressionEvaluator:
    ...
    def _replace_identifier(self, match: re.Match[str], text: str) -> str:
        ...
        if (
            ...
            and text[end] == "("
            and identifier.upper() in FORMULA_FUNCTIONS
        ):
...
class RoutineExecutor:
    ...
    def _label_index(self, routine: Routine) -> dict[str, int]:
        ...
        for index, rung in enumerate(routine.rungs):
            ...
            node = rung.nodes[0]
            if (
                isinstance(node, InstructionCall)
                and node.opcode == "LBL"
                and node.operands
            ):
    ...
    def _execute_rung(
        self, nodes: tuple[Node, ...], condition_in: bool, ctx: ScanContext
    ) -> bool:
    ...
    def _execute_branch(
        self, branch: Branch, condition_in: bool, ctx: ScanContext
    ) -> bool:
        results = []
    ...
    def _execute_instruction(
        self, instruction: InstructionCall, condition_in: bool, ctx: ScanContext
    ) -> bool:
        ...
        if opcode == "CMP":
            return bool(condition_in) and _coerce_bool(
                self.expression_evaluator.evaluate(operands[0], ctx)
            )
        ...
        if opcode == "COP":
            if condition_in:
                values = self._read_block(
                    operands[0], _coerce_int(ctx.resolve_operand(operands[2])), ctx
                )
        ...
        if opcode == "FLL":
            if condition_in:
                ...
                self._write_block(
                    operands[1],
                    [_deep_copy(fill_value) for _ in range(count)],
                    ctx,
                    fill_mode=True,
                )
            return bool(condition_in)
    ...
    def _execute_compare(
        self, opcode: str, operands, condition_in: bool, ctx: ScanContext
    ) -> bool:
        if not condition_in:
            return False
        left = ctx.resolve_operand(operands[0])
    ...
    def _execute_pid(self, operands, ctx: ScanContext):
        ...
        if _coerce_bool(control.get("SWM", False)):
            ...
            if _coerce_float(control.get("UPD", 0.0)) > 0.0:
                derivative = _coerce_float(control.get("KD", 0.0)) * (
                    previous_pv - process_variable
                )
            output = (
                _coerce_float(control.get("BIAS", 0.0))
                + proportional
                + integral
                + derivative
            )
    ...
    def _write_block(
        self, path: str, values, ctx: ScanContext, fill_mode: bool = False
    ):
        ...
        for offset, value in enumerate(values):
            if start_index + offset >= len(array):
                break
            item = (
                _zero_like(template) if fill_mode and value == 0 else _deep_copy(value)
            )
            if (
                fill_mode
                and value == 0
                and isinstance(template, (dict, list, PLCStruct))
            ):
    ...
    def _resolve_jsr_target(self, routine_name: str, ctx: ScanContext):
        if ctx.current_program is not None:
            qualified = ctx.jsr_registry.resolve(
                f"{ctx.current_program}:{routine_name}"
            )
    ...
    def _resolve_motion_control_program(
        ...
        if ctx.current_program is not None and ctx.tag_store.exists(
            control_path, program=ctx.current_program
        ):
    ...
    def _stop_coordinate(
        self, coordinate_path: str, control_path: str, ctx: ScanContext
    ):
    ...
    def _start_axis_move(self, operands, ctx: ScanContext):
        ...
        speed = self._normalize_motion_speed(
            _coerce_float(ctx.resolve_operand(operands[4]))
        )
    ...
    def _start_coordinate_move(self, opcode: str, operands, ctx: ScanContext):
        ...
        target_positions = self._resolve_coordinate_target(
            coordinate_path, operands[3], ctx
        )
        speed_index = 7 if opcode == "MCCM" else 4
        accel_index = 9 if opcode == "MCCM" else 6
        speed = self._normalize_motion_speed(
            _coerce_float(ctx.resolve_operand(operands[speed_index]))
        )
        acceleration = _coerce_float(ctx.resolve_operand(operands[accel_index]))
        start_positions = tuple(
            _coerce_float(ctx.get_value(path)) for path in target_paths
        )
        distance = math.sqrt(
            sum(
        ...
        motion = ActiveMotion(
            ...
            coordinate_path=coordinate_path,
            command_name=opcode,
            direction=_coerce_int(ctx.resolve_operand(operands[6]))
            if opcode == "MCCM"
            else None,
        )
        ...
        if coordinate_path in ctx.runtime_state.coordinate_moves:
            ...
            coordinate["MovePendingQueueFullStatus"] = (
                coordinate_path in ctx.runtime_state.coordinate_pending_moves
            )
            motion.pending = True
    ...
    def _start_axis_jog(self, operands, ctx: ScanContext):
        ...
        speed = self._normalize_motion_speed(
            _coerce_float(ctx.resolve_operand(operands[3]))
        )
    ...
    def _start_axis_registration_move(self, operands, ctx: ScanContext):
        ...
        axis = _deep_copy(ctx.get_value(axis_path))
        axis["CommandAcceleration"] = 0.0
        axis["ActualVelocity"] = (target - start_position) / max(
            ctx.scan_dt_seconds, 1e-6
        )
        axis["MoveStatus"] = True
        ctx.set_value(axis_path, axis)
    ...
    def _motion_scan_count(
        self, distance: float, speed: float, ctx: ScanContext
    ) -> int:
    ...
    def _resolve_coordinate_target(
        self, coordinate_path: str, operand: str, ctx: ScanContext
    ) -> tuple[float, ...]:
        path = str(operand)
        ...
        if (
            coordinate_path == "X_Y"
            and ctx.exists("X_POSITION")
            and ctx.exists("Y_POSITION")
        ):
            return (
    ...
    def _clear_component_velocities(
        self, component_paths: tuple[str, ...], ctx: ScanContext
    ):
    ...
    def _advance_coordinate_moves(self, ctx: ScanContext):
        ...
        for coordinate_path in completed:
            ...
            pending = ctx.runtime_state.coordinate_pending_moves.pop(
                coordinate_path, None
            )
            coordinate = _deep_copy(ctx.get_value(coordinate_path))
            if pending is None:
    ...
    def _finish_motion(self, motion: ActiveMotion, ctx: ScanContext):
        for component_path, target in zip(
            motion.component_paths, motion.target_positions
        ):
            ctx.set_value(component_path, target)
