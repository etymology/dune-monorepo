...
def simplify_file(
    path: str | Path, *, write_changes: bool = False
) -> FileSimplificationResult:
...
def _duplicate_blocker_reason(nodes: tuple[Node, ...]) -> str | None:
    ...
    if unsupported_opcode is not None:
        return (
            f"rung contains opcode {unsupported_opcode} that is not safe to duplicate"
        )
...
def _helper_lowering_plan(nodes: tuple[Node, ...]) -> HelperLoweringPlan | None:
    branch_indexes = [
        index for index, node in enumerate(nodes) if isinstance(node, Branch)
    ]
    ...
    if not suffix_nodes:
        return None
    if any(
        not isinstance(node, InstructionCall) or node.opcode not in CONDITION_OPCODES
    ...
    for branch_nodes in branch_node.branches:
        for expanded_path in _expand_nodes(branch_nodes):
            if any(
                not isinstance(node, InstructionCall)
                or node.opcode not in CONDITION_OPCODES
        ...
        lowered.append(
            Rung(
                nodes=tuple(
                    plan.prefix
                    + branch_path
                    + (InstructionCall(opcode="OTL", operands=(helper_name,)),)
                )
            )
        )
    lowered.append(
        Rung(
            nodes=(InstructionCall(opcode="XIC", operands=(helper_name,)),)
            + plan.suffix
        )
    )
    return lowered
...
def _instruction_reads_and_writes(
    ...
    if opcode in {"XIC", "XIO", "OTL", "OTU"}:
        reads = (
            _extract_tag_references(operands[0]) if opcode in {"XIC", "XIO"} else set()
        )
    ...
    if opcode in {"EQU", "GEQ", "GRT", "LEQ", "LES", "NEQ"}:
        return _extract_tag_references(operands[0]) | _extract_tag_references(
            operands[1]
        ), set()
    if opcode == "LIM":
        return (
...
def _extract_tag_references(text: str) -> set[str]:
    ...
    for match in IDENTIFIER_PATTERN.finditer(scrubbed):
        ...
        if (
            ...
            and scrubbed[end] == "("
            and name.upper() in FORMULA_FUNCTIONS
        ):
            continue
        references.add(name)
    return references
...
def _expand_nodes(nodes: tuple[Node, ...]) -> tuple[tuple[Node, ...], ...]:
    ...
    for node in nodes:
        if isinstance(node, Branch):
            ...
            expanded = [
                left + right for left in expanded for right in branch_expansions
            ]
            continue
