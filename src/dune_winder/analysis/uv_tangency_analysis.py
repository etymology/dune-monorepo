...
AVAILABLE_SITE_IDS = tuple(
    spec.site_id
    for spec in _SITE_SPECS["U"]
    if spec.site_id in {site.site_id for site in _SITE_SPECS["V"]}
)
...
def _load_machine_calibration(
    path: str | Path | None,
) -> tuple[MachineCalibration, Path | None]:
    if path is None:
...
def _render_lines(layer: str, script_variant: str = "default") -> list[str]:
    if layer == "U":
        if str(script_variant).strip().lower() not in ("", "default"):
            raise UVTangencyAnalysisError(
                "The U layer does not support script variants."
            )
...
def _replay_site_reports(
    ...
    line_text_by_index = {
        recipe_index: line_text
        for _site_id, (recipe_index, line_text) in site_line_numbers.items()
    }
...
def build_uv_tangency_report(
    ...
    *,
    ...
    if machine_calibration is None:
        machine_calibration, machine_path = _load_machine_calibration(
    ...
    return {
        ...
        "machineCalibrationPath": str(machine_path)
        if machine_path is not None
        else None,
        "layerCalibrationPath": str(calibration_path)
