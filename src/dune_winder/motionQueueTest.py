...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=TESTABLE_TERM_TYPES,
        help=(
            "Termination type applied at tangential merges. Defaults to --term-type."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    start_xy = (
        None if args.start_x is None else (float(args.start_x), float(args.start_y))
    )
    ...
    term_types = (
        list(TESTABLE_TERM_TYPES) if args.sweep_term_types else [args.term_type]
    )
    ...
    def build_case_segments(term_type: int) -> tuple[list[MotionSegment], float]:
        ...
        if (
            ...
            and after_caps
            and (
                not math.isinf(float(args.v_x_max))
                or not math.isinf(float(args.v_y_max))
            )
        ):
            print(
        ...
        tangential_term_type = (
            term_type
            if args.tangential_term_type is None
            else args.tangential_term_type
        )
