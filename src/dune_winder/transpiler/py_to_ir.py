...
class PythonToIR(ast.NodeVisitor):
  ...
  def _handle_list_param(
    ...
    for arg in func_def.args.args:
      ...
      if ann.startswith("list[") or ann.startswith("List["):
        ...
        in_params.extend(
          [(f"{arg.arg}_x", rx), (f"{arg.arg}_y", ry), (f"{arg.arg}_valid", rv)]
        )
  ...
  def _alloc_return_regs(
    ...
    if ann.startswith("tuple[float"):
      ...
      regs = [
        self.alloc.alloc(PLCType.REAL, f"{func_def.name}_ret_{i}") for i in range(parts)
      ]
      scope.ret_regs = regs
  ...
  def _conv_expr_stmt(self, call: ast.Call, scope: _Scope) -> list[IRNode]:
    ...
    if (
      isinstance(call.func, ast.Attribute) and call.func.attr == "append" and call.args
    ):
      inner = call.args[0]
    ...
    if isinstance(node.test, ast.UnaryOp) and isinstance(node.test.op, ast.Not):
      if isinstance(node.test.operand, ast.Name):
        ...
        if name == scope.list_var:
          cond: Condition = Cmp(
            "==", RegExpr(scope.vars[self.seg_count_var]), Const(0, PLCType.DINT)
          )
          then = []
          for s in node.body:
