...
class SimulatedPLC(PLC):
    ...
    def _completePendingMove(self, moveType: int):
        ...
        if moveType == self.MOVE_SEEK_XZ:
            ...
            if self._isXYLimitViolation(
                xTarget, float(self._tagValues["Y_axis.ActualPosition"])
            ):
                self._setError(3003)
                return
            ...
            zTarget = float(zTarget)
            if self._isXYLimitViolation(
                float(self._tagValues["X_axis.ActualPosition"]), yTarget
            ):
                self._setError(3003)
                return
            if self._isZLimitViolation(zTarget):
            ...
            xTarget = float(
                self._tagValues.get(
                    "X_POSITION", self._tagValues["X_axis.ActualPosition"]
                )
            )
            yTarget = float(
                self._tagValues.get(
                    "Y_POSITION", self._tagValues["Y_axis.ActualPosition"]
                )
            )
            ...
            xTarget = (
                float(self._tagValues["X_axis.ActualPosition"])
                + self._signedSpeed("X_SPEED", "X_DIR") * 0.1
            )
            yTarget = (
                float(self._tagValues["Y_axis.ActualPosition"])
                + self._signedSpeed("Y_SPEED", "Y_DIR") * 0.1
            )
            if self._isXYLimitViolation(xTarget, yTarget):
                self._setError(3003)
                return
            ...
            zTarget = float(
                self._tagValues.get(
                    "Z_POSITION", self._tagValues["Z_axis.ActualPosition"]
                )
            )
            if self._isZLimitViolation(zTarget):
                self._setError(5003)
            ...
            zTarget = (
                float(self._tagValues["Z_axis.ActualPosition"])
                + self._signedSpeed("Z_SPEED", "Z_DIR") * 0.1
            )
            if self._isZLimitViolation(zTarget):
                self._setError(5003)
                return
    ...
    def _setAxisMovement(self, isMoving: bool):
        ...
        if isMoving and self._pendingMoveType in (self.MOVE_JOG_XY, self.MOVE_SEEK_XY):
            self._tagValues["X_axis.ActualVelocity"] = self._signedSpeed(
                "X_SPEED", "X_DIR"
            )
            self._tagValues["Y_axis.ActualVelocity"] = self._signedSpeed(
                "Y_SPEED", "Y_DIR"
            )
            self._tagValues["Z_axis.ActualVelocity"] = 0.0
            ...
            self._tagValues["X_axis.ActualVelocity"] = 0.0
            self._tagValues["Y_axis.ActualVelocity"] = 0.0
            self._tagValues["Z_axis.ActualVelocity"] = self._signedSpeed(
                "Z_SPEED", "Z_DIR"
            )
        else:
            self._tagValues["X_axis.ActualVelocity"] = 0.0
