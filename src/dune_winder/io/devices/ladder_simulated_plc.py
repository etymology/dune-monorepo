...
class LadderSimulatedPLC(SimulatedPLC):
    ...
    _ROUTINES_TO_LOAD = _SCAN_ORDER + (("queued_motion", "CapSegSpeed"),)
    ...
    _ALIASES_TO_MACHINE_BITS = {
        alias: bit for bit, aliases in _MACHINE_BIT_ALIASES.items() for alias in aliases
    }
    ...
    def set_tag(self, name: str, value: Any, override=None):
        with self._lock:
            ...
            if shouldOverride:
                self._overrides[tagName] = (
                    self._coerceBit(value)
                    if self._machineBitIndex(tagName) is not None
                    else value
                )
    ...
    def _load_routines(self):
        ...
        for programName, routineName in self._ROUTINES_TO_LOAD:
            ...
            if self._routine_backend == "imperative":
                loaded = load_imperative_routine_from_source(
                    transpile_routine_to_python(routine)
                )
    ...
    def _register_jsr_targets(self):
        for (programName, routineName), routine in self._routines.items():
            ...
            if routineName not in {"CapSegSpeed"}:
                self._jsr_registry.register(
                    routineName,
                    lambda ctx, loaded=routine: self._execute_loaded_callable(
                        loaded, ctx
                    ),
                )
    ...
    def _reset_runtime_structures(self):
        for definition in self._metadata.controller_tags.values():
            self._reset_tag_definition(
                definition.name, definition.program, definition.data_type_name
            )
        for program in self._metadata.programs.values():
            for definition in program.tags.values():
                self._reset_tag_definition(
                    definition.name, program.name, definition.data_type_name
                )

    # ---------------------------------------------------------------------
    def _reset_tag_definition(
        self, name: str, program: str | None, dataTypeName: str | None
    ):
    ...
    def _apply_compatibility_state(self):
        queueActive = bool(self._ctx.get_value("CurIssued")) or bool(
            self._ctx.get_value("NextIssued")
        )
        queueActive = queueActive or bool(self._ctx.get_value("X_Y.MovePendingStatus"))
        state = int(self._ctx.get_value("STATE"))
        if queueActive and state not in {
            self.STATE_ERROR,
            self.STATE_EOT,
            self.STATE_HMI_STOP,
        }:
    ...
    def _apply_latch_stub(self) -> bool:
        ...
        if moveType == self.MOVE_HOME_LATCH or state == self.STATE_LATCH_HOMEING:
            ...
            self._ctx.set_value(
                "MACHINE_SW_STAT[0]", bool(self._ctx.get_value("LATCH_ACTUATOR_HOMED"))
            )
    ...
    def _writeTag(self, tagName, value):
        ...
        if tagName == "MOVE_TYPE":
            moveType = int(value)
            if moveType == self.MOVE_SEEK_XZ and not bool(
                self._readTagValue("Y_XFER_OK")
            ):
                ...
                self._ctx.set_value("MOVE_TYPE", self.MOVE_RESET)
                return
            if moveType == self.MOVE_SEEK_YZ and not bool(
                self._readTagValue("X_XFER_OK")
            ):
                self._ctx.set_value("ERROR_CODE", 5003)
                self._ctx.set_value("STATE", self.STATE_ERROR)
                self._ctx.set_value("NEXTSTATE", self.STATE_ERROR)
            ...
            if nextState is not None:
                ...
                moveType == self.MOVE_RESET
                and int(self._ctx.get_value("STATE")) == self.STATE_ERROR
            ):
                self._ctx.set_value("NEXTSTATE", self.STATE_READY)
            return
