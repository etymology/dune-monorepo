...
class PlcManifest:
    ...
    def update_tag_metadata(self, program_name: str | None) -> str | None:
        ...
        if program_name is None:
            ...
            self._data.setdefault("programs", {}).setdefault(program_name, {})[
                "tag_metadata"
            ] = entry
        return hash_value
        ...
        for program_name in sorted(all_programs):
            ...
            if program_dir.is_dir():
                for routine_dir in program_dir.iterdir():
                    if (
                        routine_dir.is_dir()
                        and (routine_dir / "studio_copy.rllscrap").exists()
                    ):
            ...
            for routine_name in sorted(all_routines):
                ...
                captured_at = (
                    routine_entry.get("hashed_at", "(none)")
                    if routine_entry
                    else "(none)"
                )
                ...
                if not rllscrap_path.exists():
                    ...
                    state = (
                        "ok" if current_hash == routine_entry["hash"] else "modified"
                    )
...
def _print_status_table(rows: list[StatusRow]) -> None:
    ...
    for row in rows:
        print(
            fmt.format(
                row.location, row.category, row.stored_hash, row.captured_at, row.state
            )
        )
...
def main(argv=None):
    ...
    scan_parser = subparsers.add_parser(
        "scan", help="Hash all .rllscrap files and update manifest."
    )
    ...
    status_parser = subparsers.add_parser(
        "status", help="Print artifact freshness table."
    )
    ...
    if args.command == "scan":
        ...
        gaps = [
            r
            for r in rows
            if r.category == "rllscrap" and r.state in ("new", "modified", "missing")
        ]
        if gaps:
