...
def _read_struct_array(driver, tag_definition, udts_by_name):
    ...
    index_lengths = [
        dimension for dimension in dimensions[:array_dimensions] if int(dimension) > 0
    ]
...
def fetch_and_write_tag_values(plc_path, output_root=DEFAULT_OUTPUT_ROOT):
    ...
    for json_file, (payload, _) in tags_by_file.items():
        updated_payload = apply_tag_values_to_payload(
            payload, values_by_tag, generated_at
        )
...
def build_argument_parser():
    ...
    parser.add_argument(
        "plc_path", help="PLC connection path or IP address for pycomm3."
    )
    parser.add_argument(
        "--output-root",
        type=Path,
