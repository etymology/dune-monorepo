...
def build_request_from_form(form: _FormState) -> UvHeadTargetRequest:
  return UvHeadTargetRequest(
    ...
    wrapped_pin=form.wrapped_pin_var.get(),
...
def format_result_summary(result: UvHeadTargetResult) -> str:
  return "\n".join(
    (
      f"Site: {result.site_label}",
      f"Wrap side: {result.wrap_side}",
      f"Inferred G103 pair: {result.request.wrapped_pin}, {result.inferred_pair_pin}",
...
def _collect_draw_points(result: UvHeadTargetResult) -> list[Point2D]:
  ...
  return [
    ...
    Point2D(result.wrapped_pin_point.x, result.wrapped_pin_point.y),
    Point2D(result.inferred_pair_pin_point.x, result.inferred_pair_pin_point.y),
...
def draw_result(canvas: tk.Canvas, result: UvHeadTargetResult) -> None:
  ...
  wrapped_pin = Point2D(result.wrapped_pin_point.x, result.wrapped_pin_point.y)
  inferred_pair_pin = Point2D(result.inferred_pair_pin_point.x, result.inferred_pair_pin_point.y)
  ...
  _draw_labeled_point(canvas, project, wrapped_pin, label="wrapped", color="#0f766e", radius=4.0)
  _draw_labeled_point(canvas, project, inferred_pair_pin, label="pair", color="#0f766e", radius=4.0)
...
def _build_form(root: tk.Misc) -> _FormState:
  ...
  tk.Label(controls, text="Wrapped Pin").grid(row=6, column=0, sticky="w", pady=(10, 0))
  tk.Entry(controls, textvariable=wrapped_pin_var).grid(row=7, column=0, sticky="ew")
  ...
  form = _FormState(
    ...
    wrapped_pin_var=wrapped_pin_var,
    error_var=error_var,
  ...
  tk.Label(controls, textvariable=error_var, fg="#b91c1c", justify="left", wraplength=240).grid(row=9, column=0, sticky="ew", pady=(10, 0))
