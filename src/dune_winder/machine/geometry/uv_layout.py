...
class UvLayerLayout:
  ...
  def _build_boards(self) -> tuple[UvBoard, ...]:
    ...
    boards: list[UvBoard] = []
    for board_index, (start_pin, end_pin) in enumerate(_pairwise(self.endpoint_pins), start=1):
  ...
  def _build_nominal_positions(self) -> dict[str, Point3D]:
    ...
    grids = (
      ("A", self.geometry.gridBack, z_front, self.geometry.startPinBack, self.geometry.directionBack),
      ("B", self.geometry.gridFront, z_back, self.geometry.startPinFront, self.geometry.directionFront),
    )
    for family, grid, depth, start_pin, direction in grids:
      x_value = 0.0
      y_value = 0.0
      pin_number = int(start_pin)
      for count, x_increment, y_increment, x_offset, y_offset, _orientation in grid:
        x_value += x_offset
        y_value += y_offset
  ...
  def board_lookup(
    ...
    if normalized_face not in FACE_ORDER:
      raise ValueError(
        "Board face must be one of head, bottom, foot, or top."
      )
