...
def _tangent_candidates_for_pin_pair(
  ...
  if _length_2d(Point2D(point_b.x - point_a.x, point_b.y - point_a.y)) <= _AXIS_EPSILON:
    raise UvWrapGeometryError("Cannot compute a tangent for coincident pin centers.")
...
def _compute_arm_corrected_outbound(
  *,
  ...
  if direction_length <= _AXIS_EPSILON:
    raise UvWrapGeometryError("Arm correction requires a non-degenerate tangent line.")
  ...
  if same_side:
    ...
    tangent_point_a, tangent_point_b, clipped_start, clipped_end = (
      _select_tangent_solution(
        ...
        anchor_tangent_sides=tangent_sides(normalized_layer, normalized_anchor_pin),
        wrapped_pin_point=target_point_2d,
        wrapped_tangent_sides=tangent_sides(normalized_layer, normalized_target_pin),
      )
    )
