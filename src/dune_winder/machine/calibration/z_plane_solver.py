...
_FIT_RANK_ERROR = (
    "Need at least 3 non-collinear projected-tangent measurements to fit a Z plane."
)
...
def build_layer_z_plane_observation(
    ...
    *,
    ...
    tangent_a = np.array(
        [
            float(view.raw_result.tangent_point_a.x),
            float(view.raw_result.tangent_point_a.y),
        ],
        dtype=float,
    )
    tangent_b = np.array(
        [
            float(view.raw_result.tangent_point_b.x),
            float(view.raw_result.tangent_point_b.y),
        ],
        dtype=float,
    )
    transfer = np.array(
    ...
    if denominator <= 1e-12:
        raise ValueError(
            "Projected tangent geometry is degenerate for this measurement."
        )
...
def _load_layer_calibration_from_path(
    path: str | Path | None,
...
def fit_layer_z_plane_from_observations(
    ...
    *,
    ...
    coefficients, rank, residual_sum_squares = _fit_plane_from_observations(
        observations
    )
    result.rank = rank
...
def fit_layer_z_plane(
    ...
    *,
    ...
    normalized_layers = {
        _normalize_layer(measurement.layer) for measurement in measurements
    }
