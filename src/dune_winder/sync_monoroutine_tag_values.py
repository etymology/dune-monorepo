...
def _resolve_monoroutine_sources(plc_root: Path) -> tuple[list[dict], dict]:
    monoroutine_payload = _load_json(
        _monoroutine_program_tags_path(MONOROUTINE_PLC_ROOT)
    )
...
def _read_live_values(
    ...
    *,
    ...
    for resolution in matched_resolutions:
        preferred_fqn = _program_scoped_fqn(
            target_program_name, resolution["monoroutine_tag"]
        )
    ...
    try:
        ...
        for preferred_fqn, tag_definition in definitions_by_fqn.items():
            ...
            if error is None:
                continue
            if tag_definition.get("tag_type") != "struct" or not tag_definition.get(
                "udt_name"
            ):
                continue
            values_by_fqn[preferred_fqn] = _read_tag_value_with_fallback(
                driver, tag_definition, udts_by_name
            )
        ...
        for resolution in matched_resolutions:
            ...
            fallback_value, fallback_error = _read_tag_value_with_fallback(
                driver, fallback_definition, udts_by_name
            )
...
def _attach_source_metadata(
    resolutions: list[dict], monoroutine_payload: dict, plc_root: Path
) -> list[dict]:
    source_index, _payloads = _build_source_name_index(plc_root)
    definitions_by_source = {}
    for entries in source_index.values():
        for entry in entries:
            definitions_by_source[(entry["program_name"], entry["tag_name"])] = entry[
                "tag_definition"
            ]
    ...
    for resolution in resolutions:
        ...
        if resolution["status"] == "matched":
            tag_definition = definitions_by_source[
                (resolution["source_program"], resolution["source_tag"])
            ]
...
def sync_monoroutine_tag_values(
    ...
    *,
    ...
    resolutions = _attach_source_metadata(
        resolutions, context["monoroutine_payload"], plc_root
    )
    ...
    report = {
        ...
        "summary": {
            **context["summary"],
            ...
            "read_errors": sum(
                1 for value, error in values_by_fqn.values() if error is not None
            ),
        },
