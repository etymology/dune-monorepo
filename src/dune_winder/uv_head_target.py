...
def translated_anchor_to_target_projection(
  ...
  *,
  ...
  if raw.alternating_plane is not None:
    translated_head = _translated_point2(view.interpreter_head_point, delta_x, delta_y)
    translated_wire = _translated_point2(view.interpreter_wire_point, delta_x, delta_y)
    return {
  ...
  translated_head = _shared_choose_outbound_intercept(
    locus_origin,
    Point2D(float(locus_origin.x) + float(direction.x), float(locus_origin.y) + float(direction.y)),
    clipped[0],
    clipped[1],
  )
