...
class MotionService:
    ...
    def jogXY(self, xVelocity, yVelocity, acceleration=None, deceleration=None):
        ...
        if (
            ...
            0 == xVelocity and 0 == yVelocity and self._controlStateMachine.isJogging()
        ):
    ...
    def manualSeekZ(self, position, velocity=None):
        ...
        if self._controlStateMachine.isReadyForMovement():
            ...
            if (
                zPosition < safetyLimits.zlimit_front
                or zPosition > safetyLimits.zlimit_rear
            ):
                self._log.add(
                    LOG_NAME,
                    "JOG",
                    "Manual move Z ignored - position "
                    + str(zPosition)
                    + " is outside limits ["
                    + str(safetyLimits.zlimit_front)
                    + ", "
                    + str(safetyLimits.zlimit_rear)
                    + "].",
                    [position, velocity],
                )
                isError = True
    ...
    def seekPin(self, pin, velocity):
        ...
        if calibration:
            ...
            if calibration.getPinExists(pinNameA) and calibration.getPinExists(
                pinNameB
            ):
                self._log.add(
                    LOG_NAME,
