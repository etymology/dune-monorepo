...
class Process:
    ...
    def __init__(
        ...
        io.head.setExtendedAndRetracted(
            machineCalibration.zFront, machineCalibration.zBack
        )
    ...
    def _getActiveLayerCalibration(self, layer):
        ...
        if calibration is None:
            raise CalibrationNotLoadedError(
                "No layer calibration is loaded for active layer "
                + requestedLayer
                + "."
            )
    ...
    def jogXY(self, xVelocity, yVelocity, acceleration=None, deceleration=None):
        return self._motionService().jogXY(
