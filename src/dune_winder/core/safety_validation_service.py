...
def _calibration_float(
    calibration: MachineCalibration, key: str, default: float
) -> float:
    value = None
    try:
...
class SafetyValidationService:
    ...
    def __init__(
        ...
        self._queuedMotionZCollisionThreshold = _calibration_float(
            ...
            _calibration_float(cal, "zBack", 0.0),
        )
        self._arcMaxStepRad = _calibration_float(
            cal, "arcMaxStepRad", math.radians(3.0)
        )
        ...
        self._transferZoneHeadMinX = _calibration_float(
            cal, "transferZoneHeadMinX", 400.0
        )
        self._transferZoneHeadMaxX = _calibration_float(
            cal, "transferZoneHeadMaxX", 500.0
        )
        self._transferZoneFootMinX = _calibration_float(
            cal, "transferZoneFootMinX", 7100.0
        )
        self._transferZoneFootMaxX = _calibration_float(
            cal, "transferZoneFootMaxX", 7200.0
        )
    ...
    def current_motion_safety_limits(self) -> MotionSafetyLimits:
        return MotionSafetyLimits(
            ...
            queued_motion_z_collision_threshold=float(
                self._queuedMotionZCollisionThreshold
            ),
