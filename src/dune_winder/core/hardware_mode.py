    ...
    def update(self):
        ...
        if self.io.plc.isNotFunctional():
            ...
            if xWorking and not self.isX_AxisWorking:
                self.log.add(
                    self.__class__.__name__, "HARD_ERROR", "X-axis fault clear."
                )
                self.isX_AxisWorking = True

            if yWorking and not self.isY_AxisWorking:
                self.log.add(
                    self.__class__.__name__, "HARD_ERROR", "Y-axis fault clear."
                )
                self.isY_AxisWorking = True

            if zWorking and not self.isZ_AxisWorking:
                self.log.add(
                    self.__class__.__name__, "HARD_ERROR", "Z-axis fault clear."
                )
                self.isZ_AxisWorking = True
            ...
            if self.io.isFunctional():
                self.log.add(
                    self.__class__.__name__, "HARD_ERROR", "All hardware ready."
                )
