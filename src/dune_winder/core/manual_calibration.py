        ...
        for parameter in grid:
            ...
            for _ in range(count):
                calibration.setPinLocation(
                    side + str(pinNumber),
                    Location(
                        round(xValue, 5) + origin.x, round(yValue, 5) + origin.y, depth
                    ),
                )
...
class ManualCalibration:
    ...
    def _loadPersistedGXSession(self, session, data):
        ...
        for referenceId in GX_REFERENCE_IDS:
            ...
            if storedReference is not None:
                reference["rawCameraX"] = self._optionalFloat(
                    storedReference.get("rawCameraX")
                )
                reference["rawCameraY"] = self._optionalFloat(
                    storedReference.get("rawCameraY")
                )
                reference["offsetX"] = self._optionalFloat(
                    storedReference.get("offsetX")
                )
                reference["offsetY"] = self._optionalFloat(
                    storedReference.get("offsetY")
                )
    ...
    def _persistGXSession(self, session):
        try:
            ...
            self._process._log.add(
                "ManualCalibration",
                "DRAFT_SAVE",
                "Failed to save manual calibration draft for layer "
                + session.layer
                + ".",
    ...
    def _loadPersistedSession(self, session):
        ...
        try:
            ...
            self._process._log.add(
                "ManualCalibration",
                "DRAFT_LOAD",
                "Failed to load manual calibration draft for layer "
                + session.layer
                + ".",
                [draftPath, exception],
            )
        ...
        try:
            if session.mode == "gx":
                ...
                session.cameraOffsetX, session.cameraOffsetY = (
                    self._sharedCameraOffset()
                )
            ...
            self._process._log.add(
                "ManualCalibration",
                "DRAFT_LOAD",
                "Failed to restore manual calibration draft for layer "
                + session.layer
                + ".",
                [draftPath, exception],
            )
    ...
    def _persistSession(self, session, persistBaseline=False):
        ...
        try:
            ...
            if persistBaseline or not os.path.isfile(
                self._draftBaselinePath(session.layer)
            ):
                ...
                for pinName in session.baselineCalibration.getPinNames():
                    baselineLocation = session.baselineCalibration.getPinLocation(
                        pinName
                    )
                    baselineCalibration.setPinLocation(
                        pinName,
                        Location(
                            baselineLocation.x, baselineLocation.y, baselineLocation.z
                        ),
                    )
    ...
    def _mutationGuard(self):
        if not self._process.controlStateMachine.isReadyForMovement():
            return self._errorResult(
                "Machine is not ready for manual calibration moves."
            )
    ...
    def _buildPredictionContext(self, session):
        ...
        for pin in sorted(session.measuredPins):
            ...
            transformPairs.append(
                (baseline.x, baseline.y, measurement["wireX"], measurement["wireY"])
            )
        ...
        for pin in sorted(session.measuredPins):
            ...
            transformedX, transformedY = _apply_transform(
                transform, baseline.x, baseline.y
            )
            measurement = session.measuredPins[pin]
            exactPins[pin] = {
    ...
    def _interpolateSideResidual(self, context, pin):
        ...
        if len(anchors) == 0:
            ...
            nearest = min(
                ...
                key=lambda anchor: _cyclic_pin_distance(
                    pin, anchor[0], metadata["pinMax"]
                ),
            )
            ...
            if leftPin <= pin <= rightPin:
                return (
                    _interpolate_residual(
                        leftPin, leftResidual, rightPin, rightResidual, pin
                    ),
                    "side",
                )
    ...
    def _setBoardCheck(self, session, pin, status, wireX, wireY):
        session.boardChecks[pin] = self._boardCheckEntry(
            session, pin, status, wireX, wireY
        )
    ...
    def getState(self):
        ...
        if not enabled:
            if layer is None:
                disabledReason = (
                    "Load a U, V, X, or G recipe to use manual calibration."
                )
            else:
        ...
        filteredBootstrapPins = self._getFilteredBootstrapPins(
            metadata, session.skipWrapPins
        )
        bootstrapPins = []
        ...
        bootstrapDone = len(
            [item for item in bootstrapPins if item["status"] != "pending"]
        )
        boardCheckDone = len(
            [item for item in boardChecks if item["status"] != "pending"]
        )
        ...
        for session in self._sessions.values():
            ...
            for pin, measurement in session.measuredPins.items():
                if (
                    ...
                    measurement["wireY"] = (
                        measurement["rawCameraY"] + measurement["offsetY"]
                    )
                    if pin in LAYER_METADATA[session.layer]["endpointInfo"]:
                        self._setBoardCheck(
                            session,
    ...
    def updateReferencePoint(self, referenceId, wireX, wireY):
        ...
        session = self._getSession(layer)
        previous = session.references.get(
    ...
    def setCornerOffset(self, offsetId, value):
        ...
        return self._okResult(
            {"offsetId": offsetId, "value": session.offsets[offsetId]}
        )
    ...
    def generateRecipeFile(self):
        ...
        if self._gxReferenceCount(session) != len(GX_REFERENCE_IDS):
            return self._errorResult(
                "Record both the head and foot reference points first."
            )
    ...
    def deleteMeasuredPin(self, pin):
        ...
        if pin not in session.measuredPins:
            return self._errorResult(
                "Pin " + str(pin) + " does not have a measurement."
            )

        del session.measuredPins[pin]
        if (
            pin in session.boardChecks
            and session.boardChecks[pin]["status"] == "adjusted"
        ):
    ...
    def markBoardCheck(self, pin, status):
        ...
        if pin in session.measuredPins:
            ...
            if not prediction.get("ok", False):
                return prediction
            self._setBoardCheck(
                session, pin, "ok", prediction["wireX"], prediction["wireY"]
            )
    ...
    def saveLive(self):
        ...
        for pinName in session.baselineCalibration.getPinNames():
            ...
            if pinName.startswith("B"):
                ...
                calibration.setPinLocation(
                    pinName, Location(wireX, wireY, baselineLocation.z)
                )
