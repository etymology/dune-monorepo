...
class GCodePlaybackService:
    ...
    def __init__(
        ...
        machineCalibrationGetter: Callable[[], Optional[MachineCalibration]] = lambda: (
            None
        ),
    ):
    ...
    def stopNextLine(self):
        if (
            self._controlStateMachine.isInMotion()
    ...
    def getLayerCalibration(self):
        ...
        def pin_sort_key(name):
            text = str(name)
            prefix_rank = (
                0 if text.startswith("A") else 1 if text.startswith("B") else 2
            )
            suffix = text[1:]
            try:
        ...
        if self._gCodeHandler.isG_CodeLoaded():
            ...
            if not isError:
                self._log.add(
                    LOG_NAME,
                    "RUN_TO",
                    "G-Code finial line changed from "
                    + str(initialRunTo)
                    + " to "
                    + str(line),
                    [initialRunTo, line],
    ...
    def executeG_CodeLine(self, line: str):
        ...
        if not self._controlStateMachine.isReadyForMovement():
            ...
            xyf = (
                r"(\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *[F]\d{1,4}\ *$)"
            )
            fxy = (
                r"(\ *[F]\d{1,4}\ *[X]\d{1,4}(\.\d{1,2})?\ *[Y]\d{1,4}(\.\d{1,2})?\ *$)"
            )
            ...
            xzf = (
                r"(\ *[X]\d{1,4}(\.\d{1,2})?\ *[Z]\d{1,3}(\.\d{1,2})?\ *[F]\d{1,4}\ *$)"
            )
            fxz = (
                r"(\ *[F]\d{1,4}\ *[X]\d{1,4}(\.\d{1,2})?\ *[Z]\d{1,3}(\.\d{1,2})?\ *$)"
            )
            yz = r"(\ *[Y]\d{1,4}(\.\d{1,2})?\ *[Z]\d{1,3}(\.\d{1,2})?\ *$)"
            yzf = (
                r"(\ *[Y]\d{1,4}(\.\d{1,2})?\ *[Z]\d{1,3}(\.\d{1,2})?\ *[F]\d{1,4}\ *$)"
            )
            fyz = (
                r"(\ *[F]\d{1,4}\ *[Y]\d{1,4}(\.\d{1,2})?\ *[Z]\d{1,3}(\.\d{1,2})?\ *$)"
            )
            f_only = r"(\ *[F]\d{1,4}(\.\d{1,2})?\ *$)"
            a_only = r"(\ *[A]\d{1,4}(\.\d{1,2})?\ *$)"
            ...
            absoluteXYMovePattern = "|".join(
                [xy, x_only, y_only, xyf, fxy, xf, fx, yf, fy]
            )
            ...
            y = yPosition
            isXYMove = re.match(
                absoluteXYMovePattern + "|" + relativeXYMovePattern, line
            )
            ...
            for cmd in codeLineSplit:
                ...
                if "Z" in cmd and re.match(
                    ...
                    if (
                        z_target < self._safety.zlimit_front
                        or z_target > self._safety.zlimit_rear
                    ):
                        error = (
            ...
            if error is not None:
                ...
                if errorData:
                    ...
                    self._controlStateMachine.dispatch(
                        ManualModeEvent(executeGCode=True)
                    )
