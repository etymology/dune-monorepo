...
class WindMode(StateMachineState):
    ...
    # ---------------------------------------------------------------------
    def __init__(self, stateMachine, state: int, io: BaseIO, log: Log):
        """
        Constructor.
        ...
        if self._stopRequested:  # and self.io.Tension_10N.get() :
            ...
            if self.stateMachine.gCodeHandler.isG_CodeError():
                ...
                self._log.add(
                    ...
                    "WIND_ERROR",
                    "G-Code error.  "
                    + self.stateMachine.gCodeHandler.getG_CodeErrorMessage(),
            ...
            if isDone:
                # Log message that wind is complete.
                self._log.add(
                    self.__class__.__name__, "WIND", "G-Code execution complete"
                )
