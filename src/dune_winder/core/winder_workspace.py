...
class WinderWorkspace:
    ...
    _LAYER_FILE_WRITERS = {
        "V": lambda path, archive: write_v_template_file(
            path, archive_directory=archive
        ),
        "U": lambda path, archive: write_u_template_file(
            path, archive_directory=archive
        ),
    ...
    def loadRecipe(self, layer=None, recipeFile=None, startingLine=-1):
        ...
        if (
            startingLine == -1
            and recipeFile is not None
            and recipeFile in self._lineHistory
        ):
    ...
    def _buildUvSegments(self):
        if self._layer not in ("U", "V"):
            raise ValueError(
                "UV segment search is only available for the U and V layers."
            )
    ...
    def _getWrapStartLine(self, wrap):
        ...
        expression = re.compile(
            r"\(\s*" + str(int(wrap)) + r"\s*,\s*1\b", re.IGNORECASE
        )
        for index, line in enumerate(self._recipe.getLines(), start=1):
            if expression.search(line):
                return index
    ...
    def refreshCalibrationIfChanged(self):
        ...
        if self._calibration is None:
            self._loadCalibrationFromDisk(
                "no calibration was active in the G-Code system"
            )
            return
