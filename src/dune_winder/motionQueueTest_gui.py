...
class WaypointPlannerApp(tk.Tk):
    def __init__(self, plc_path: str, machine_calibration: str) -> None:
        ...
        self.v_x_max, self.v_y_max = _load_axis_velocity_limits(
            machine_calibration or None
        )
        ...
        self.min_arc_radius_var = tk.StringVar(
            value=f"{DEFAULT_WAYPOINT_MIN_ARC_RADIUS:.1f}"
        )
        ...
        self.min_segment_length_var = tk.StringVar(
            value=f"{DEFAULT_MIN_SEGMENT_LENGTH:.1f}"
        )
    ...
    def _build_layout(self) -> None:
        ...
        row = 0
        ttk.Label(
            controls, text="Planner Settings", font=("Segoe UI", 11, "bold")
        ).grid(row=row, column=0, columnspan=2, sticky="w")
        row += 1

        row = self._entry_row(controls, row, "PLC path", self.plc_path_var)
        ttk.Label(controls, text="Waypoint order").grid(
        ...
        ttk.Label(controls, text="Term type").grid(
            row=row, column=0, sticky="w", pady=(8, 0)
        )
        ...
        row = self._entry_row(
            controls, row, "Min segment length", self.min_segment_length_var
        )
        ...
        ttk.Button(
            button_bar_1, text="Undo Last", command=self._undo_last_waypoint
        ...
        ttk.Label(
            controls, textvariable=self.stats_var, wraplength=320, justify="left"
        ).grid(row=row, column=0, columnspan=2, sticky="w", pady=(10, 0))
        row += 1
        ttk.Label(
            controls, textvariable=self.status_var, wraplength=320, justify="left"
                ...
                if center is not None:
                    ...
                    if r0 > 1e-9 and r1 > 1e-9:
                        ...
                        if sweep is not None:
                            radius = 0.5 * (r0 + r1)
                            steps = max(
                                4, int(math.ceil(abs(sweep) / math.radians(4.0)))
                            )
    ...
    def _path_length(
        ...
        if start_xy is None:
            prev = MotionSegment(
                seq=segments[0].seq - 1, x=segments[0].x, y=segments[0].y
            )
        else:
    ...
    def _draw_machine_overlay(self) -> None:
        ...
        if (
            transfer_x_max > limits.limit_left
            and limits.limit_top > limits.transfer_y_threshold
        ):
            tx0, ty0 = self._world_to_canvas(
                limits.limit_left, limits.transfer_y_threshold
            )
    ...
    def _draw_grid(self) -> None:
        ...
        while x_tick <= limits.limit_right + 1e-6:
            ...
            self.canvas.create_text(
                px0 + 3, py0 - 10, text=f"{x_tick:.0f}", fill="#94a3b8", anchor="w"
            )
            x_tick += x_step
        ...
        while y_tick <= limits.limit_top + 1e-6:
            ...
            self.canvas.create_line(px0, py0, px1, py1, fill="#1f2937")
            self.canvas.create_text(
                px0 + 3, py0 - 3, text=f"{y_tick:.0f}", fill="#94a3b8", anchor="w"
            )
            y_tick += y_step
    ...
    def _refresh_canvas(self) -> None:
        ...
        if self.segments:
            ...
            if len(poly) >= 2:
                ...
                self.canvas.create_oval(
                    spx - 6, spy - 6, spx + 6, spy + 6, fill="#22c55e", outline=""
                )
                self.canvas.create_oval(
                    epx - 6, epy - 6, epx + 6, epy + 6, fill="#ef4444", outline=""
                )
        ...
        for i, (x, y) in enumerate(self.waypoints, start=1):
            ...
            if i == len(self.waypoints):
                color = "#ef4444"
            self.canvas.create_oval(
                px - 5, py - 5, px + 5, py + 5, fill=color, outline="#0f172a"
            )
            self.canvas.create_text(
                px + 9, py - 10, text=str(i), fill="#dbeafe", anchor="w"
            )
        ...
        if self.live_position_xy is not None:
            ...
            if (
                ...
                self.canvas.create_oval(
                    px - 6, py - 6, px + 6, py + 6, outline="#22c55e", width=2
                )
                self.canvas.create_text(
                    px + 10,
                    py + 10,
    ...
    def _replan(self) -> None:
        ...
        try:
            planner_waypoints = _planner_waypoints(
                self.live_position_xy, self.waypoints
            )
            if planner_waypoints is None or len(planner_waypoints) < 2:
                raise ValueError(
                    "Planner requires the live position and at least one distinct destination."
                )
            ...
            min_arc_radius = self._parse_float(
                self.min_arc_radius_var.get(), "Min arc radius"
            )
            ...
            def build_planned_segments(
                ...
                planned_segments = [
                    replace(seg, speed=speed) for seg in planned_segments
                ]
    ...
    def _execute_path(self) -> None:
        ...
        if self.live_position_xy is None or self.start_xy is None:
            self._set_status(
                "Live position is required before execution.", is_error=True
            )
            return
        ...
        try:
            while not self._position_tracker_stop.is_set():
                ...
                try:
                    ...
                    safety_tripped = _coerce_to_bool(
                        _read_plc_tag_value(plc, TAG_SAFETY_TRIPPED)
                    )
                    self.after(
                        0,
                        lambda x=x_val, y=y_val, tripped=safety_tripped: (
