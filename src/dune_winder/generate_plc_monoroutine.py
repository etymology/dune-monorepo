...
SEGQUEUE_FIELD_PATTERN = r"\.[A-Za-z_][A-Za-z0-9_]*(?:\[[^\]]+\])*(?:\.[A-Za-z_][A-Za-z0-9_]*(?:\[[^\]]+\])*)*"
...
def _collect_collision_names(program_payloads: dict[str, dict]) -> set[str]:
    ...
    return {name for name, programs in name_to_programs.items() if len(programs) > 1}


def _build_program_tag_maps(
...
def _rename_routine(
    ...
    *,
    ...
    label_map = {label: f"{label_prefix}_{label}" for label in sorted(labels)}
    return replace(
        routine,
        rungs=tuple(
...
def _inline_standalone_jsrs(
    rungs: tuple[Rung, ...], inline_map: dict[str, tuple[Rung, ...]]
) -> tuple[Rung, ...]:
    flattened: list[Rung] = []
    for rung in rungs:
...
def _prepare_move_xy_main(
    ...
    for rung in main_routine.rungs:
        if any(
            node.opcode == "JSR" and node.operands[0] == "xy_speed_regulator"
            for node in _iter_nodes(rung.nodes)
        ):
...
def _build_monoroutine(
    parser: RllParser, rename_maps: dict[str, dict[str, str]]
) -> Routine:
    rungs: list[Rung] = []
...
def _collect_array_index_usage(
    routine_text: str, tag_name: str
) -> tuple[set[int], bool]:
...
def _trim_tag_dimensions(raw_tag: dict, new_name: str, routine_text: str) -> dict:
    ...
    literal_indexes, saw_non_literal = _collect_array_index_usage(
        routine_text, new_name
    )
    ...
    for program_name in PROGRAM_ORDER:
        ...
        for raw_tag in payload.get("program_tags", []):
            ...
            if new_name in seen_names:
                raise ValueError(
                    f"duplicate monoroutine tag name after merge: {new_name}"
                )
...
def _generate_program_tags_payload(
    ...
    return {
        ...
        "program_tags": _merge_program_tags(
            program_payloads, rename_maps, routine_text
        ),
    }
...
def main(argv=None) -> int:
    ...
    messages.extend(
        _write_or_check_json(
            ...
            _generate_program_tags_payload(
                program_payloads, rename_maps, monoroutine_text
            ),
            check=args.check,
        )
    )
