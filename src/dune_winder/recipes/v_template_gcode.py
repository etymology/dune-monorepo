...
from dune_winder.recipes.template_gcode_foot_pauses import (
    apply_anchor_to_target_foot_pauses,
)
...
PULL_IN_IDS = ("Y_PULL_IN", "X_PULL_IN", "HEAD_ARM_LENGTH")
DEFAULT_PULL_INS = {
    "Y_PULL_IN": Y_PULL_IN,
    "X_PULL_IN": X_PULL_IN,
...
V_WRAP_BASE_SCRIPT = compile_template_script(
    (
        ...
        "if near_comb(799 + wrap): emit G113 PTOLERANT G103 PF${799 + wrap} PF${798 + wrap} PX G105 ${coord('PX', COMB_PULL)}",
        ...
        "if near_comb(1999 + wrap): emit G113 PTOLERANT G103 PB${1998 + wrap} PB${1999 + wrap} PX G105 ${coord('PX', -COMB_PULL)}",
    )
)
...
V_WRAP_NORMAL_TAIL_SCRIPT = compile_template_script(
    (
        ...
        "if near_comb(399 + wrap): emit G113 PTOLERANT G103 PB${399 + wrap} PB${400 + wrap} PX G105 ${coord('PX', COMB_PULL)}",
    )
)
...
V_WRAP_FINAL_TAIL_SCRIPT_XZ = compile_template_script(
    (
        "emit G113 PPRECISE G103 PB2398 PB2399 PY G105 PY0 G111",
...
def iter_v_wrap_primary_sites(
    *,
    ...
    for wrap_number in range(1, WRAP_COUNT + 1):
        ...
        execute_template_script(
            final_tail_script if wrap_number == WRAP_COUNT else normal_tail_script,
            environment=environment,
            output_lines=wrap_lines,
            line_builder=_token_line,
            transfers=transfers,
    ...
    *,
    ...
    if script_variant == SCRIPT_VARIANT_WRAPPING:
        ...
        if add_foot_pauses_value:
            lines = apply_anchor_to_target_foot_pauses(lines, layer="V")
