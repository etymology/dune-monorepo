...
class CommandRegistry:
    ...
    def _dispatch(self, name: str, args: dict):
        ...
        if name not in self._commands:
            raise CommandDispatchException(
                "UNKNOWN_COMMAND", "Unknown command: " + str(name)
            )
    ...
    def execute(self, name: str, args=None):
        ...
        try:
            ...
            return CommandRegistry._error(
                "INTERNAL_ERROR", "Internal error while executing command."
            )
    ...
    def executeRequest(self, payload):
        ...
        if not isinstance(name, str) or len(name.strip()) == 0:
            return CommandRegistry._error(
                "BAD_REQUEST", "Field 'name' must be a non-empty string."
            )
    ...
    def executeBatchRequest(self, payload):
        ...
        if not isinstance(requests, list):
            return CommandRegistry._error(
                "BAD_REQUEST", "Field 'requests' must be a list."
            )
