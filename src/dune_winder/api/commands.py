...
def build_command_registry(
    ...
    registry.register(
        "process.manual_seek_xy_named", process_manual_seek_xy_named, True
    )
    ...
    registry.register(
        "process.manual_head_position", process_manual_head_position, True
    )

    def process_seek_pin(args):
        _validateArgs(args, required=("pin", "velocity"))
        return process.seekPin(
            _asString(args["pin"], "pin").upper(),
            _asFloat(args["velocity"], "velocity"),
        )
    ...
    def _active_layer_calibration_or_raise(layer):
        ...
        if handler is not None and hasattr(handler, "getLayerCalibration"):
            ...
            if (
                ...
                and str(calibration.getLayerNames()).strip().upper() == layer
            ):
                return calibration
    ...
    def _sync_layer_calibration_handlers(calibration):
        ...
        workspace_handler = getattr(
            getattr(process, "workspace", None), "_gCodeHandler", None
        )
    ...
    registry.register(
        ...
        lambda args: (
            _validateArgs(args),
            process.machineGeometryCalibration.getState(),
        )[1],
        False,
    )
    ...
    def v_template_set_transfer_pause(args):
        _validateArgs(args, required=("enabled",))
        return process.vTemplateRecipe.setTransferPause(
            _asBool(args["enabled"], "enabled")
        )
    ...
    def v_template_set_add_foot_pauses(args):
        _validateArgs(args, required=("enabled",))
        return process.vTemplateRecipe.setAddFootPauses(
            _asBool(args["enabled"], "enabled")
        )
    ...
    registry.register(
        "process.v_template.set_include_lead_mode",
        v_template_set_include_lead_mode,
        True,
    )
    ...
    registry.register(
        "process.v_template.set_strip_g113_params",
        v_template_set_strip_g113_params,
        True,
    )
    ...
    def u_template_set_transfer_pause(args):
        _validateArgs(args, required=("enabled",))
        return process.uTemplateRecipe.setTransferPause(
            _asBool(args["enabled"], "enabled")
        )
    ...
    def u_template_set_add_foot_pauses(args):
        _validateArgs(args, required=("enabled",))
        return process.uTemplateRecipe.setAddFootPauses(
            _asBool(args["enabled"], "enabled")
        )
    ...
    registry.register(
        "process.u_template.set_include_lead_mode",
        u_template_set_include_lead_mode,
        True,
    )
    ...
    registry.register(
        "process.set_position_logging", process_set_position_logging, True
    )
