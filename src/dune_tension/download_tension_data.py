...
def _extract_wire_rows(action: dict[str, Any], apa_uuid: str, apa_name: str) -> list[WireRow]:
  ...
  rows: list[WireRow] = []
  for side_key, side_name in (("measuredTensions_sideA", "a"), ("measuredTensions_sideB", "b")):
...
def _insert_apa(conn: sqlite3.Connection, apa: dict[str, Any]) -> tuple[str, str]:
  ...
  apa_name = str((apa.get("data") or {}).get("componentName") or apa.get("componentName") or apa_uuid)
  conn.execute(
      """
...
def download(output_path: Path, include_empty: bool = False) -> dict[str, int]:
  ...
  try:
    apa_uuids = [apa for apa in GetListOfComponents(APA_COMPONENT_TYPE, connection, headers) if apa]
    tension_action_ids = [action_id for action_id in GetListOfActions(TENSION_ACTION_TYPE, connection, headers) if action_id]
