...
def _compute_tensions(
  ...
  for side in ["A", "B"]:
    ...
    line_data.append(selected[["wire_number", "tension"]].assign(side_label=side_label))
...
def build_summary_plot_figure(
  ...
  *,
  ...
  for side_label, group in line_df.groupby("side_label"):
    ...
    rolling_mean = (
      sorted_group["tension"].rolling(window=20, center=True, min_periods=20).mean()
    )
    ...
    resid_data.append(pd.DataFrame({"residual": residuals, "side_label": side_label}))
  ...
  if resid_data:
    ...
    for side_label, group in resid_df.groupby("side_label"):
      ...
      if np.isfinite(resid_stats["sigma"]):
        ...
        resid_stats_text_parts.append(f"{side_label}: σ={resid_stats['sigma']:.2f}")
