...
class StreamingMeasurementController:
  ...
  def _persist_final_result(
    ...
    avg_frequency = float(
      np.average(
        candidate.pitch_estimates, weights=np.maximum(candidate.pitch_confidences, 1e-6)
      )
    )
    ...
    try:
      ...
      for candidate in candidates.values():
        if (
          ...
          accepted_results.append(self._persist_final_result(candidate, "stream_sweep"))
        else:
