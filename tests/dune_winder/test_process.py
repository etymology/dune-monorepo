...
class FakeWorkspaceForRefreshMessages:
    ...
    def _reloadRecipeWhileStopped(
        self, previousLines, reloadedLines, cachedPinPair=None
    ):
        from dune_winder.core.winder_workspace import WinderWorkspace
    ...
    def _findReloadLineByPinPair(self, previousLines, reloadedLines, currentLine):
        ...
        return WinderWorkspace._findReloadLineByPinPair(
            self, previousLines, reloadedLines, currentLine
        )
    ...
    def _findClosestPinPairLine(self, lines, pinPair, currentLine):
        ...
        return WinderWorkspace._findClosestPinPairLine(
            self, lines, pinPair, currentLine
        )
...
class ProcessSnapshotTests(unittest.TestCase):
    ...
    def test_refresh_before_execution_returns_error_when_recipe_refresh_fails(self):
        process = object.__new__(Process)
        process.workspace = FakeAPARefresh(
            recipeError=RuntimeError("recipe changed badly")
        )
        process._log = FakeLog()
    ...
    def test_get_layer_calibration_returns_normalized_absolute_locations(self):
        ...
        process.gCodeHandler = type(
            "Handler", (), {"getLayerCalibration": lambda self: calibration}
        )()
        ...
        self.assertAlmostEqual(
            payload["locations"]["B400"]["x"], expected.x + 12.5, places=6
        )
        self.assertAlmostEqual(
            payload["locations"]["B400"]["y"], expected.y - 7.25, places=6
        )
    ...
    def test_get_layer_calibration_rejects_wrong_active_layer(self):
        ...
        process.gCodeHandler = type(
            "Handler", (), {"getLayerCalibration": lambda self: None}
        )()
    ...
    def test_refresh_before_execution_returns_actionable_missing_recipe_message(self):
        ...
        self.assertEqual(
            result,
            WinderWorkspace._missingRecipeMessage(
                process.workspace, "C:/recipes/V-layer.gc"
            ),
        )
        self.assertEqual(len(process._log.entries), 1)
    ...
    def test_gcode_reload_rejects_line_count_changes(self):
        handler = GCodeHandler(
            _ReloadGuardFakeIO(), _ReloadGuardFakeCalibration(), None
        )
    ...
    def test_refresh_before_execution_returns_error_when_recipe_line_count_changes(
        self,
    ):
        from dune_winder.core.winder_workspace import WinderWorkspace
        ...
        with tempfile.TemporaryDirectory() as tempDir:
            ...
            process.workspace._gCodeHandler.loadG_Code(
                ["G1 X1", "G1 X2"], calibration=None
            )
            process.workspace._controlStateMachine = _ReloadGuardControlStateMachine(
                WindMode()
            )
            process._log = FakeLog()
    ...
    def test_refresh_before_execution_reseeks_by_pin_pair_when_stopped_and_line_count_changes(
        self,
    ):
        with tempfile.TemporaryDirectory() as tempDir:
            recipePath = os.path.join(tempDir, "V-layer.gc")
    ...
    def test_refresh_before_execution_reseeks_anchor_to_target_by_pin_pair_when_line_count_matches(
        self,
    ):
        with tempfile.TemporaryDirectory() as tempDir:
    ...
    def test_refresh_before_execution_anchor_to_target_uses_last_pin_pair_when_current_line_has_none(
        self,
    ):
        with tempfile.TemporaryDirectory() as tempDir:
            recipePath = os.path.join(tempDir, "U-layer.gc")
    ...
    def test_refresh_before_execution_returns_actionable_missing_calibration_message(
        self,
    ):
        from dune_winder.core.winder_workspace import WinderWorkspace
...
class ProcessManualGCodeTests(unittest.TestCase):
    def _build_process_for_manual_gcode(self, x_position=10.0, y_position=20.0):
        ...
        process._motion = MotionService(
            process._io,
            process._log,
            process.controlStateMachine,
            safety,
            process.gCodeHandler,
            None,
            process._xBacklash,
            lambda: None,
        )
    ...
    def test_execute_manual_gcode_rejects_pivot_keepout_crossing(self):
        process = self._build_process_for_manual_gcode(
            x_position=400.0, y_position=1400.0
        )
    ...
    def test_manual_seek_xy_accepts_safe_target(self):
        process = self._build_process_for_manual_gcode(
            x_position=400.0, y_position=100.0
        )
    ...
    def test_manual_seek_xy_reversal_uses_unbiased_raw_target(self):
        process = self._build_process_for_manual_gcode(
            x_position=112.0, y_position=100.0
        )
        process._xBacklash.setBacklashMm(2.0)
    ...
    def test_manual_seek_xy_skips_sub_resolution_noop_without_entering_manual_mode(
        self,
    ):
        process = self._build_process_for_manual_gcode(
            x_position=112.0, y_position=100.0
