...
class LadderSimulatedPlcTests(unittest.TestCase):
    ...
    def _enqueue_segment(
        self, plc: LadderSimulatedPLC, req_id: int, segment: MotionSegment
    ):
        plc.set_tag(
            "IncomingSeg",
            {
    ...
    def _expected_capped_speed(
        ...
        max_tx, max_ty = _segment_tangent_component_bounds(
            start_xy[0], start_xy[1], segment
        )
    ...
    def test_xz_seek_respects_transfer_override(self):
        ...
        self._advance_until(
            plc, lambda: plc.get_tag("STATE") in (plc.STATE_ERROR, plc.STATE_READY)
        )
    ...
    def test_hmi_stop_request_interrupts_queued_motion_and_aborts_queue(self):
        ...
        self._advance_until(
            plc, lambda: plc.get_tag("STATE") == plc.STATE_QUEUED_MOTION
        )
    ...
    def test_queue_start_caps_diagonal_segment_before_cmd_a_issue(self):
        ...
        expected_speed = self._expected_capped_speed(
            (0.0, 0.0), segment, v_x_max, v_y_max
        )
    ...
    def test_queue_segment_is_capped_before_start_pulse(self):
        ...
        self.assertAlmostEqual(
            plc.get_tag("SegQueue[0].Speed"), expected_speed, places=6
        )
        self._assert_capped_to_axis_components(
    ...
    def test_queue_circle_segment_is_capped_before_start_pulse(self):
        ...
        expected_speed = self._expected_capped_speed(
            start_xy, segment, v_x_max, v_y_max
        )
        self.assertFalse(plc.get_tag("StartQueuedPath"))
        self.assertFalse(plc.get_tag("CurIssued"))
        self.assertEqual(plc.get_tag("QueueCount"), 1)
    ...
    def test_queue_start_caps_pending_segment_before_cmd_b_issue(self):
        ...
        self._advance_until(
            plc,
            lambda: (
                plc.get_tag("CurSeg.Seq") == first.seq
                and plc.get_tag("NextSeg.Seq") == second.seq
            ),
        )

        expected_first = self._expected_capped_speed(
            (0.0, 0.0), first, v_x_max, v_y_max
        )
        expected_second = self._expected_capped_speed(
            (first.x, first.y), second, v_x_max, v_y_max
        )
