...
class MotionQueueTestGuiTests(unittest.TestCase):
    ...
    def test_planner_waypoints_deduplicates_first_destination_matching_live_position(
        self,
    ):
        planner_waypoints = _planner_waypoints(
            (10.0, 20.0),
    ...
    def test_live_position_matches_plan_start_within_tolerance(self):
        self.assertTrue(
            _live_position_matches_plan_start(
                (10.0, 20.0), (10.05, 20.05), tolerance_mm=0.1
            )
        )
    ...
    def test_live_position_rejects_plan_start_outside_tolerance(self):
        self.assertFalse(
            _live_position_matches_plan_start(
                (10.0, 20.0), (10.2, 20.0), tolerance_mm=0.1
            )
        )
    ...
    def test_planned_speed_summary_reports_capped_speed_range(self):
        ...
        self.assertEqual(
            text, "requested_speed=1000.0 planned_speed[min/max]=600.0/825.0"
        )
    ...
    def test_required_waypoint_min_arc_radius_uses_dynamic_radius_for_arcs(self):
        ...
        required_radius = _required_waypoint_min_arc_radius(
            [arc], requested_min_arc_radius
        )

        self.assertEqual(
            required_radius, _segment_dynamic_min_radius(arc, requested_min_arc_radius)
        )
