...
class CommandValidationTests(unittest.TestCase):
    ...
    def test_machine_compute_roller_y_cal_accepts_jerk_keyword(self):
        registry, _, _, _, _, _ = build_registry_fixture()
        response = registry.executeRequest(
            {
                "name": "machine.compute_roller_y_cal",
                "args": {
                    "gcode_line": "~anchorToTarget(B1201,B2001,offset=(1.5,-2),hover=True,inTwoMoves=True,jerk=gentle)",
                    "actual_x": 3297.0,
                    "actual_y": 2683.0,
                    "layer": "U",
                },
    ...
    def test_execute_gcode_line_returns_validation_error_when_process_reports_failure(
        self,
    ):
        registry, process, _, _, _, _ = build_registry_fixture()
