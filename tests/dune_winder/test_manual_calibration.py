...
class ManualCalibrationTests(unittest.TestCase):
    ...
    def test_layer_metadata_uses_expected_pin_counts_and_bootstrap(self):
        ...
        for layer in ("U", "V"):
            ...
            for sideIndex, side in enumerate(("head", "bottom", "foot", "top")):
                sideBoards = [
                    board for board in metadata["boards"] if board["side"] == side
                ]
                sideMidpoint = (
                    sideBoards[0]["startPin"] + sideBoards[-1]["endPin"]
                ) / 2.0
    ...
    def test_default_session_starts_nominal_and_recalibration_uses_live_geometry(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            liveCalibration.save(
                process._workspaceCalibrationDirectory,
                "U_Calibration.xml",
                "LayerCalibration",
            )
    ...
    def test_shared_camera_offset_seeds_machine_calibration_from_legacy_u_config(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            self.assertAlmostEqual(
                process._machineCalibration.cameraWireOffsetX, 11.5, places=6
            )
            self.assertAlmostEqual(
                process._machineCalibration.cameraWireOffsetY, -6.5, places=6
            )
    ...
    def test_set_camera_offset_updates_machine_calibration_and_all_layer_keys(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            self.assertTrue(result["ok"])
            self.assertAlmostEqual(
                process._machineCalibration.cameraWireOffsetX, 12.5, places=6
            )
            self.assertAlmostEqual(
                process._machineCalibration.cameraWireOffsetY, -7.5, places=6
            )
    ...
    def test_draft_persists_without_overwriting_live_calibration(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            draftDirectory = os.path.join(
                process.workspace.getPath(), "ManualCalibration"
            )
            self.assertTrue(
                os.path.isfile(os.path.join(draftDirectory, "U_Draft.json"))
            )
            self.assertTrue(
    ...
    def test_xg_state_uses_loaded_runtime_calibration_references_when_present(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            self.assertAlmostEqual(
                state["references"]["head"]["wireX"], 612.5, places=6
            )
            self.assertAlmostEqual(
                state["references"]["head"]["wireY"], 182.25, places=6
            )
            self.assertAlmostEqual(
                state["references"]["foot"]["wireX"], 7012.0, places=6
            )
            self.assertAlmostEqual(
                state["references"]["foot"]["wireY"], 184.75, places=6
            )
    ...
    def test_xg_draft_persists_without_writing_live_recipe(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            self.assertTrue(
                os.path.isfile(os.path.join(draftDirectory, "G_Draft.json"))
            )
            self.assertFalse(
                os.path.isfile(
                    os.path.join(process.workspace._recipeDirectory, "G-layer.gc")
                )
            )
    ...
    def test_hashless_calibration_refreshes_runtime_calibration_when_file_changes(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            baselineBack = calibration.getPinLocation("B1")
            calibration.save(
                calibrationDirectory, "U_Calibration.xml", "LayerCalibration"
            )
            ...
            self.assertTrue(
                any(
                    ...
                    for entry in apa._log.entries
                )
            )
            self.assertTrue(
                any(
                    "Reloaded calibration file" in entry[2]
                    for entry in apa._log.entries
                )
            )
