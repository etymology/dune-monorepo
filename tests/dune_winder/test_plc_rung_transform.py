...
class PlcRungTransformTests(unittest.TestCase):
    def test_transform_text_converts_bracketed_conditions_and_normalizes_spacing(self):
        source = (
            "   [XIC QueueFault,XIC MoveA.ER ,XIC MoveB.ER ];  XIC (SomeTag,OtherTag)"
        )
    ...
    def test_transform_text_preserves_parentheses_in_cmp_expression(self):
        ...
        self.assertEqual(
            result, 'CMP "ABS(X_axis.ActualPosition-xz_position_target[0])<0.1" '
        )
    ...
    def test_transform_text_applies_bracketed_condition_rewrite_before_flattening(self):
        ...
        self.assertEqual(
            result, "Prefix BST XIC CurIssued NXB XIO QueueFault BND Suffix"
        )
