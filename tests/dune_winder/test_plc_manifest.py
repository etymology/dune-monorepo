...
class ComputeMetadataHashTests(unittest.TestCase):
    ...
    def test_ignores_generated_at_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_json(
                path,
                _make_program_tags_payload(generated_at="2026-01-01T00:00:00+00:00"),
            )
            hash1 = compute_metadata_hash(path)

            _write_json(
                path,
                _make_program_tags_payload(generated_at="2026-06-01T00:00:00+00:00"),
            )
    ...
    def test_ignores_values_generated_at_change(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_json(
                path,
                _make_program_tags_payload(
                    values_generated_at="2026-01-01T00:00:00+00:00"
                ),
            )
            hash1 = compute_metadata_hash(path)
            ...
            _write_json(
                path,
                _make_program_tags_payload(
                    values_generated_at="2026-06-01T00:00:00+00:00"
                ),
            )
            hash2 = compute_metadata_hash(path)
...
class ComputeRllscrapHashTests(unittest.TestCase):
    def test_crlf_and_lf_produce_same_hash(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                compute_rllscrap_hash(lf_path), compute_rllscrap_hash(crlf_path)
            )
...
class PlcManifestUpdateTests(unittest.TestCase):
    def test_update_tag_metadata_writes_hash_and_generated_at(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_json(
                json_path,
                _make_program_tags_payload(generated_at="2026-03-20T00:00:00+00:00"),
            )
    ...
    def test_update_tag_values_returns_none_when_values_generated_at_absent(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIsNone(result)
            self.assertNotIn(
                "tag_values", manifest._data.get("programs", {}).get("TestProgram", {})
            )
...
class PlcManifestStatusTests(unittest.TestCase):
    ...
    def _find_row(self, rows, location, category):
        return next(
            (r for r in rows if r.location == location and r.category == category), None
        )
            ...
            "programs": {
                "TestProgram": {
                    ...
                    "program_tags": [
                        {
                            "name": "TagA",
                            "fully_qualified_name": "Program:TestProgram.TagA",
                            "tag_type": "atomic",
                            "data_type_name": "DINT",
                        }
                    ],
