...
class EmbeddedModuleTests(unittest.TestCase):
    ...
    def test_gcode_handler_base_main_block_cases(self):
        ...
        self.assertEqual(
            Location(handler._x, handler._y, handler._z), Location(10, 10, 10)
        )
    ...
    def test_g109_g103_lines_emit_motion_trace_logging(self):
        ...
        with self.assertLogs(
            "dune_winder.gcode.handler_base", level="INFO"
        ) as captured:
        ...
        expected_center = expected_pin_a.center(expected_pin_b).add(
            handler.layerCalibration.offset
        )
        ...
        self.assertAlmostEqual(
            anchor["calibrationSpace"]["x"], expected_anchor.x, places=6
        )
        self.assertAlmostEqual(
            anchor["wireSpace"]["x"],
        ...
        self.assertAlmostEqual(
            pin_a["calibrationSpace"]["x"], expected_pin_a.x, places=6
        )
        self.assertAlmostEqual(
            pin_b["calibrationSpace"]["x"], expected_pin_b.x, places=6
        )
        self.assertEqual(payload["pinCenter"]["axes"], "XY")
        self.assertAlmostEqual(
            payload["pinCenter"]["wireSpace"]["x"], expected_center.x, places=6
        )
        self.assertAlmostEqual(
            payload["resultingTarget"]["x"], expected_center.x + 5.0, places=6
        )
        self.assertAlmostEqual(
            payload["resultingTarget"]["y"], expected_center.y, places=6
        )
    ...
    def test_g109_g103_motion_trace_is_written_to_log_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_directory:
            ...
            with open(log_path, encoding="utf-8") as input_file:
                lines = [
                    entry.strip() for entry in input_file.readlines() if entry.strip()
                ]
