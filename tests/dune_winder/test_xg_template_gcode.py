...
class XGTemplateGCodeTests(unittest.TestCase):
    ...
    def test_render_x_layer_matches_programmatic_description(self):
        ...
        self.assertEqual(lines[1], "N1 G206 P0\n")
        self.assertEqual(lines[2], "N2 " + self.MERGE + "(1,1) X635.0 Y201.5\n")
        self.assertEqual(lines[3], "N3 " + self.MERGE + "(1,2) X7165.0 Y399.5\n")
        self.assertEqual(lines[4], "N4 (1,3) G206 P0\n")
        self.assertEqual(lines[5], "N5 (1,4) G206 P3\n")
        self.assertEqual(lines[6], "N6 " + self.MERGE + "(1,5) X7016.0 Y398.5\n")
        self.assertEqual(
            lines[7], "N7 " + self.MERGE + "(1,6 HEAD RESTART) X440.0 Y202.5\n"
        )
        self.assertEqual(lines[8], "N8 (1,7) G206 P3\n")
        self.assertEqual(lines[9], "N9 (1,8) G206 P0\n")
    ...
    def test_render_g_layer_uses_481_wraps_and_optional_transfer_pause(self):
        ...
        self.assertEqual(lines[1], "N1 G206 P0\n")
        self.assertEqual(lines[2], "N2 " + self.MERGE + "(1,1) X635.0 Y201.5\n")
        self.assertEqual(lines[6], "N6 (1,5) G206 P3\n")
        self.assertEqual(lines[7], "N7 " + self.MERGE + "(1,6) X7016.0 Y398.5\n")
        self.assertEqual(lines[10], "N10 (1,9) G206 P2\n")
        self.assertEqual(lines[11], "N11 (1,10) G206 P0\n")
        self.assertEqual(lines[-2], "N4811 (481,10) G206 P0\n")
    ...
    def test_write_xg_template_file_writes_recipe_header_and_body(self):
        with tempfile.TemporaryDirectory() as rootDirectory:
            ...
            result = write_xg_template_file(
                "X", outputPath, specialInputs=self._special_inputs()
            )
            ...
            self.assertEqual(
                recipe.getLines()[0], "N0 " + self.MERGE + "X440.0 Y201.5\n"
            )
            self.assertEqual(recipe.getLines()[1], "N1 G206 P0\n")
            self.assertEqual(
                recipe.getLines()[2], "N2 " + self.MERGE + "(1,1) X635.0 Y201.5\n"
            )
