...
class ControlStateMachineZMoveTests(_TagIsolationTestCase):
    ...
    def _advance_machine(
        self, io: BaseIO, machine: ControlStateMachine, scans: int = 1
    ):
        for _ in range(scans):
            io.pollInputs()
            machine.update()

    def _advance_machine_until(
        self, io: BaseIO, machine: ControlStateMachine, predicate, limit: int = 200
    ):
        for _ in range(limit):
            io.pollInputs()
            machine.update()
    ...
    def test_plc_error_forces_stop_mode_from_wind(self):
        ...
        machine.gCodeHandler = type(
            "_FakeGCodeHandler", (), {"stop": lambda self: None}
        )()
