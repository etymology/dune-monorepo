...
from dune_winder.queued_motion.plc_interface import (
    QueuedMotionPLCInterface,
    validate_queue_segment,
)
...
class _IO:
    def __init__(self, x, y, z=0.0, plc_logic=None, y_transfer_ok=True, **locks):
        ...
        self.zAxis = _Axis(z)
        self.plcLogic = (
            _QueuedMotionPLCLogic(y_transfer_ok=y_transfer_ok)
...
class QueuedMotionTests(unittest.TestCase):
    ...
    def test_gcode_builder_keeps_single_merge_line_as_queued_block(self):
        ...
        handler = GCodeHandler(
            _IO(400.0, 100.0), calibration, WirePathModel(calibration)
        )
        handler._x = 400.0
        handler._y = 100.0
        handler._z = 0.0
    ...
    def test_gcode_builder_does_not_emit_trace_during_preview(self):
        ...
        self.assertEqual(
            traces[0]["line"], "G113 PPRECISE G109 PA1200 PTR G103 PA1199 PA1198 PXY"
        )

    def test_gcode_builder_uses_live_plc_accel_and_decel_limits_for_queued_segments(
        self,
    ):
        calibration = DefaultMachineCalibration()
        ...
        handler = GCodeHandler(
            _IO(400.0, 100.0, plc_logic=plc_logic),
            calibration,
    ...
    def test_gcode_builder_defaults_queued_motion_jerk_to_separate_physical_limits(
        self,
    ):
        calibration = DefaultMachineCalibration()
        handler = GCodeHandler(
    ...
    def test_gcode_builder_uses_configured_separate_queued_motion_jerk_limits(self):
        ...
        handler = GCodeHandler(
            _IO(400.0, 100.0), calibration, WirePathModel(calibration), configuration
        )
        handler._x = 400.0
        handler._y = 100.0
        handler._z = 0.0
    ...
    def test_validate_queue_segment_rejects_nonpositive_jerk(self):
        with self.subTest("jerk_accel"):
            with self.assertRaises(ValueError):
                validate_queue_segment(
                    MotionSegment(seq=101, x=125.0, y=250.0, jerk_accel=0.0)
                )

        with self.subTest("jerk_decel"):
            with self.assertRaises(ValueError):
                validate_queue_segment(
                    MotionSegment(seq=101, x=125.0, y=250.0, jerk_decel=-1.0)
                )
    ...
    def test_single_step_g113_queues_first_planned_segment_with_full_stop(self):
        ...
        self.assertAlmostEqual(
            stepped_segment.via_center_x, full_first_segment.via_center_x, places=6
        )
        self.assertAlmostEqual(
            stepped_segment.via_center_y, full_first_segment.via_center_y, places=6
        )
    ...
    def test_single_step_g113_executes_queue_and_stops_before_next_line(self):
        ...
        expected_segment = preview_handler._build_queued_block(
            0, single_step_queue=True
        )["segments"][0]
        ...
        self.assertAlmostEqual(
            plc.get_tag("Y_axis.ActualPosition"), expected_segment.y, places=6
        )
    ...
    def test_start_queued_block_falls_back_when_queue_planner_rejects_path(self):
        ...
        with patch.object(
            handler,
            "_build_queued_block",
            side_effect=ValueError("queued path invalid"),
        ):
    ...
    def test_execute_manual_line_routes_xz_move_through_plc_logic(self):
        calibration = DefaultMachineCalibration()
        handler = GCodeHandler(
            _IO(400.0, 100.0, z=50.0), calibration, WirePathModel(calibration)
        )
    ...
    def test_execute_manual_line_rejects_xz_move_with_z_out_of_bounds(self):
        ...
        error = handler.executeG_CodeLine(
            f"X500 Z{float(calibration.zLimitRear) + 1.0}"
        )
    ...
    def test_sub_resolution_xy_move_is_treated_as_noop(self):
        ...
        handler = GCodeHandler(
            _IO(1000.0, 150.0, z=200.0), calibration, WirePathModel(calibration)
        )
    ...
    def test_gcode_builder_rejects_central_apa_motion_when_z_extended(self):
        calibration = self._z_collision_calibration()
        handler = GCodeHandler(
            _IO(1000.0, 25.0, z=200.0), calibration, WirePathModel(calibration)
        )
        handler._x = 1000.0
        handler._y = 25.0
        handler._z = 200.0
    ...
    def test_gcode_builder_allows_head_transfer_motion_when_supports_clear(self):
        calibration = self._z_collision_calibration()
        handler = GCodeHandler(
            _IO(450.0, 25.0, z=200.0), calibration, WirePathModel(calibration)
        )
        handler._x = 450.0
        handler._y = 25.0
        handler._z = 200.0
    ...
    def test_motion_queue_client_rejects_unsafe_segment_and_accepts_transfer_zone_segment(
        self,
    ):
