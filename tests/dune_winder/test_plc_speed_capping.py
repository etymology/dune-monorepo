...
def _run_session(
    session: QueuedMotionSession, clock: _FakeClock, max_ticks: int = 80
) -> None:
...
class PLCSpeedCappingTests(unittest.TestCase):
    ...
    def test_horizontal_line_write_back_called_with_capped_speed(self):
        ...
        self.assertGreater(
            len(writes), 0, "No write_seg_queue_speed calls — capping never ran"
        )
        idx, capped = writes[0]
    ...
    def test_multiple_segments_each_illegal_one_written_back(self):
        ...
        segs = [
            ...
            MotionSegment(
                seq=2, x=200.0, y=200.0, speed=50.0
            ),  # within limit → no write
        ...
        self.assertIn(
            0, written_indices, "Segment 0 (too fast) should have been written back"
        )
        self.assertIn(
            2, written_indices, "Segment 2 (too fast) should have been written back"
        )
        self.assertNotIn(
            1, written_indices, "Segment 1 (within limit) must not be written back"
        )
        for idx, speed in writes:
            self.assertLessEqual(
                speed,
                max(v_x_max, v_y_max) + 1e-6,
