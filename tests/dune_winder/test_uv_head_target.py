...
_LINE_RE = re.compile(
    r"G109\s+(P[AB]\d+)\s+P([A-Z]{2})\s+G103\s+(P[AB]\d+)\s+(P[AB]\d+)\s+PXY"
)
...
def test_compute_uv_head_target_matches_runtime_for_known_v_case():
    ...
    actual = handler._headCompensation.getActualLocation(
        handler._headCompensation.anchorPoint().copy(
            x=handler._x, y=handler._y, z=head_z
        )
    )
...
def test_compute_uv_tangent_view_accepts_any_calibrated_pin_pair(layer, pin_a, pin_b):
    ...
    assert (
        result.pin_a_point.x != result.pin_b_point.x
        or result.pin_a_point.y != result.pin_b_point.y
    )
...
def test_compute_uv_tangent_view_derives_wrap_side_for_f_pin():
    result = compute_uv_tangent_view(
        UvTangentViewRequest(layer="U", pin_a="B2001", pin_b="A800")
    )
...
def test_compute_uv_tangent_view_probes_runtime_orientation_for_non_recipe_pair():
    result = compute_uv_tangent_view(
        UvTangentViewRequest(layer="U", pin_a="B1201", pin_b="A1201")
    )
    ...
    assert (
        math.isclose(
            ...
            result.transfer_bounds.left,
            abs_tol=1e-6,
        )
        or math.isclose(
            result.runtime_outbound_intercept.x,
            result.transfer_bounds.right,
            abs_tol=1e-6,
        )
        or math.isclose(
            result.runtime_outbound_intercept.y,
            result.transfer_bounds.bottom,
            abs_tol=1e-6,
        )
        or math.isclose(
            result.runtime_outbound_intercept.y,
            result.transfer_bounds.top,
            abs_tol=1e-6,
        )
    )
...
def test_compute_uv_tangent_view_prefers_nearby_local_adjacent_pin_for_runtime_probe():
    result = compute_uv_tangent_view(
        UvTangentViewRequest(layer="V", pin_a="A1", pin_b="A2398")
    )
...
def test_compute_uv_tangent_view_builds_alternating_projection_for_yz_face():
    ...
    assert math.isclose(
        result.alternating_wrap_line_end.x, result.z_extended, abs_tol=1e-6
    )
    assert math.isclose(
        abs(
            result.alternating_anchor_segment_end.y
            - result.alternating_anchor_segment_start.y
        ),
...
def test_compute_uv_tangent_view_builds_alternating_projection_for_xz_face():
    ...
    assert math.isclose(
        abs(
            ...
            - result.alternating_anchor_segment_start.x
        ),
        result.pin_radius * 2.0,
        abs_tol=1e-6,
    )
...
def test_compute_uv_tangent_view_rejects_mixed_pair_on_different_faces():
    with pytest.raises(
        UvHeadTargetError, match="same face after converting the A pin to the B side"
    ):
        compute_uv_tangent_view(
            UvTangentViewRequest(layer="U", pin_a="B1201", pin_b="A2001")
        )
...
def test_compute_uv_tangent_view_returns_arm_geometry_from_machine_config():
    ...
    assert math.isclose(
        result.head_roller_radius,
        float(machine_calibration.headRollerRadius),
        abs_tol=1e-9,
    )
    assert math.isclose(
        result.head_roller_gap, float(machine_calibration.headRollerGap), abs_tol=1e-9
    )
    assert math.isclose(
...
def test_arm_correction_pin_direction_is_unavailable_on_axes(
    anchor_point, target_point
):
    assert (
        _arm_correction_head_shift_signs(
...
def test_compute_arm_corrected_outbound_returns_transfer_edge_point_for_selected_quarter_arc():
    ...
    assert math.isclose(corrected_outbound.x, 20.0, abs_tol=1e-6) or math.isclose(
        corrected_outbound.y, 20.0, abs_tol=1e-6
    )
...
def test_compute_uv_tangent_view_rejects_same_pin():
    with pytest.raises(UvHeadTargetError, match="must be different pins"):
        compute_uv_tangent_view(
            UvTangentViewRequest(layer="U", pin_a="B1201", pin_b="B1201")
        )
