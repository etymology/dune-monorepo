...
class GCodePlaybackServiceTests(unittest.TestCase):
    def _build_service(
        self, *, in_motion, plc_ready=True, axis_seeking=False, head_ready=True
    ):
        return GCodePlaybackService(
            ...
            _FakeLog(),
            _FakeIO(
                plc_ready=plc_ready, axis_seeking=axis_seeking, head_ready=head_ready
            ),
            safety=None,
    ...
    def test_stop_always_issues_direct_hmi_stop_and_dispatches_event_while_in_motion(
        self,
    ):
        ...
        self.assertIsInstance(
            service._controlStateMachine.dispatched[0], StopMotionEvent
        )
    ...
    def test_stop_issues_direct_hmi_stop_even_when_control_state_machine_is_idle(self):
        service = self._build_service(
            in_motion=False, plc_ready=True, axis_seeking=False, head_ready=True
        )
