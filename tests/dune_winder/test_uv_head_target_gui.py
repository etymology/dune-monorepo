...
def _sample_result() -> UvTangentViewResult:
  ...
  return UvTangentViewResult(
    ...
    runtime_line_equation=LineEquation(slope=0.611111, intercept=-0.222222, is_vertical=False),
...
def test_calculate_and_render_updates_summary_and_canvas(monkeypatch):
  ...
  assert any(call[0] == "create_text" and call[2].get("text") == "+x" for call in form.pin_a_zoom_canvas.calls if len(call) > 2)
  assert any(call[0] == "create_text" and call[2].get("text") == "-x" for call in form.pin_a_zoom_canvas.calls if len(call) > 2)
  assert any(call[0] == "create_text" and call[2].get("text") == "+y" for call in form.pin_a_zoom_canvas.calls if len(call) > 2)
  assert any(call[0] == "create_text" and call[2].get("text") == "+x" for call in form.pin_b_zoom_canvas.calls if len(call) > 2)
  assert any(call[0] == "create_text" and call[2].get("text") == "-x" for call in form.pin_b_zoom_canvas.calls if len(call) > 2)
...
def test_calculate_and_render_draws_alternating_side_view(monkeypatch):
  ...
  assert any(
    call[0] == "create_text"
    and "alternating-side view" in call[2].get("text", "")
    for call in form.canvas.calls
    if len(call) > 2
  )
  assert any(
    call[0] == "create_text"
    and "See main canvas" in call[2].get("text", "")
    for call in form.pin_a_zoom_canvas.calls
    if len(call) > 2
  )
