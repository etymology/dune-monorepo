...
class PlcLadderRuntimeTests(unittest.TestCase):
    ...
    def _assert_contexts_match(
        ...
        self.assertEqual(
            expected.runtime_state.axis_moves, actual.runtime_state.axis_moves
        )
        self.assertEqual(
            expected.runtime_state.coordinate_moves,
            actual.runtime_state.coordinate_moves,
        )
        self.assertEqual(
    ...
    def test_generated_python_matches_ast_for_branch_math_and_expression_execution(
        self,
    ):
        routine = self.parser.parse_routine_text(
            "main",
    ...
    def test_coordinate_motion_tracks_pending_status_scan_by_scan(self):
        ...
        self.assertAlmostEqual(
            self.ctx.get_value("X_axis.ActualPosition"), 200.0, places=6
        )
        self.assertAlmostEqual(
            self.ctx.get_value("Y_axis.ActualPosition"), 75.0, places=6
        )
    ...
    def test_generated_python_matches_ast_for_coordinate_motion_scan_by_scan(self):
        ...
        self._assert_ast_and_generated_match(
            routine, setup=setup, advance_runtime_scans=10
        )
