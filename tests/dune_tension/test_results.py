...
def test_derive_tension_fields_uses_explicit_zone_when_provided(monkeypatch) -> None:
    def _fail_zone_lookup(_x):
        raise AssertionError("zone lookup should not be used")

    monkeypatch.setattr(results_module, "zone_lookup", _fail_zone_lookup)
    monkeypatch.setattr(
        results_module,
        "length_lookup",
    ...
    assert derived.zone == 4
    assert derived.wire_length == 1.5
    assert derived.tension == 6.0
    assert derived.tension_pass is True
...
def test_tension_result_from_measurement_uses_explicit_zone(monkeypatch) -> None:
    def _fail_zone_lookup(_x):
        raise AssertionError("zone lookup should not be used")
