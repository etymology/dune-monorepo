...
from audioProcessing import get_pitch_crepe, get_pitch_naive_fft, get_pitch_autocorrelation
import logging
...
npz_files = [file for file in files if file.endswith('.npz') and 'UB' in file]
...
with open('results.csv', 'w', newline='') as csvfile:
    ...
    csv_writer.writerow(['Filename', 'Array Name', 'Minimum', 'Maximum', 'Crepe Frequency', 'Crepe Confidence'])
            ...
            try:
                ...
                if isinstance(array, np.ndarray) and array.ndim == 1 and array.size > 0:
                    ...
                    if os.path.exists(filename):
                        ...
                        autocorr_frequency, autocorr_confidence = get_pitch_autocorrelation(array, sample_rate)
                        crepe_frequency, crepe_confidence = get_pitch_crepe(array, sample_rate)
                        fft_frequency, fft_confidence = get_pitch_naive_fft(array, sample_rate)
                        ...
                        print(f"FFT pitch is {fft_frequency} Hz with confidence {fft_confidence}")
                        ...
                        if crepe_confidence > 0.8:
                            ...
                            ax2.set_title(f"Positive Frequency Domain of {npz_file} - {array_name}")
                            ...
                            if autocorr_frequency <= 5000:
                                ...
                                ax2.text(autocorr_frequency, max(positive_magnitudes) / 2, 
                            ...
                            if crepe_frequency <= 5000:
                                ...
                                ax2.text(crepe_frequency, max(positive_magnitudes) / 2, 
                                        f'{crepe_frequency:.2f} Hz\nConf: {crepe_confidence:.2f}', 
                                        color='g', ha='center')

                            # Plot the autocorrelation function
                            ax3.plot(autocorr)
                            ax3.set_title(f"Autocorrelation Function of {npz_file} - {array_name}")
                            ax3.set_xlabel('Lag')
