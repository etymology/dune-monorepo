...
try:  # Optional dependency - heavy ML models
    from pesto import Pesto  # type: ignore
except Exception:  # pragma: no cover - dependency may be absent
...
def compute_pesto_activation(audio: np.ndarray, cfg: PitchCompareConfig) -> Optional[np.ndarray]:
    if Pesto is None:
        ...
        return None
    # The pesto library expects audio in float32 and sample rate metadata.
    try:
        if hasattr(Pesto, "from_pretrained"):
            model = Pesto.from_pretrained(cfg.pesto_model_name)
        else:
            model = Pesto(cfg.pesto_model_name)
    except Exception as exc:  # pragma: no cover - depends on pesto installation
        print(f"[WARN] Could not initialize pesto model: {exc}")
        return None
    ...
    min_step_ms = (10.0 / cfg.min_frequency) * 1000.0
    step_ms = max(step_ms, min_step_ms)
    if hasattr(model, "compute_activation"):
        return model.compute_activation(audio, sample_rate=cfg.sample_rate, step_size=float(step_ms))
    if hasattr(model, "activation"):
        return model.activation(audio, sample_rate=cfg.sample_rate, step_size=float(step_ms))
    print("[WARN] Pesto model does not expose an activation computation method.")
    return None
