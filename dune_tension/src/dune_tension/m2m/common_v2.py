...
################################
def EditComponent(
    componentUUID, componentData_fields, componentData_values, connection, headers
):
...
#####################################################################################
def GetComponents_byTypeRecordNumber(
    componentTypeFormID, typeRecordNumber, connection, headers
):
    ...
    try:
        connection.request(
            "GET",
            f"/api/search/componentsByTypeAndNumber/{componentTypeFormID}/{typeRecordNumber}",
            headers=headers,
        )
        ...
        if len(components) == 0:
            sys.exit(
                f" GetComponents_byTypeRecordNumber() - ERROR: there are no component records with type form ID = {componentTypeFormID} and type record number = {typeRecordNumber} \n"
            )
        ...
        print(
            f" GetComponents_byTypeRecordNumber() [GET /api/search/componentsByTypeAndNumber/typeFormId/typeRecordNumber] - HTTP EXCEPTION: {e} \n"
        )
    except socket.timeout as s:
        print(
            f" GetComponents_byTypeRecordNumber() [GET /api/search/componentsByTypeAndNumber/typeFormId/typeRecordNumber] - SOCKET TIMEOUT: {s} \n"
        )
...
def GetListOfWorkflows(workflowTypeFormID, connection, headers):
    ...
    try:
        connection.request(
            "GET", "/api/workflows/" + workflowTypeFormID + "/list", headers=headers
        )
        ...
        workflowInformation = json.loads(
        ...
        print(
            f" GetListOfWorkflows() [GET /api/workflows/typeFormId/list] - HTTP EXCEPTION: {e} \n"
        )
    except socket.timeout as s:
        print(
            f" GetListOfWorkflows() [GET /api/workflows/typeFormId/list] - SOCKET TIMEOUT: {s} \n"
        )
