...
def ExtractEnvelopeResults(dataFile):
    ...
    if not dataFile_exists:
        sys.exit(
            " ExtractEnvelopeResults() - ERROR: the specified input file does not exist! \n"
        )
    ...
    print(
        " ExtractEnvelopeResults() - INFO: successfully extracted envelope analysis results"
    )
...
def ExtractPlanarityResults(dataFile):
    ...
    if not dataFile_exists:
        sys.exit(
            " ExtractPlanarityResults() - ERROR: the specified input file does not exist! \n"
        )
    ...
    df_planarity[["Tolerance", "Units", "Tolerance_Com"]] = df_planarity[
        "Tolerance"
    ...
    print(
        " ExtractPlanarityResults() - INFO: successfully extracted planarity analysis results"
    )
...
def ExtractM4HoleResults(dataFile):
    ...
    if not dataFile_exists:
        sys.exit(
            " ExtractM4HoleResults() - ERROR: the specified input file does not exist! \n"
        )
    ...
    dict_m4Holes = pd.read_excel(
        dataFile, header=0, usecols="B : L", engine="openpyxl", nrows=56
    )
    dict_m4Holes = dict_m4Holes.dropna(how="all", axis=0)
    dict_m4Holes = dict_m4Holes.dropna(how="all", axis=1)
    dict_m4Holes.columns = [
        "x",
        "y",
        "z",
        "dist_1stHole",
    ...
    dict_m4Holes = dict_m4Holes.round(
        pd.Series(
            [3, 3, 3, 3, 3, 3, 3],
            index=[
                "x",
    ...
    print(
        " ExtractM4HoleResults() - INFO: successfully extracted M4 hole measurement results"
    )
