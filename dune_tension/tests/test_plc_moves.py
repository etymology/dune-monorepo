...
def test_goto_xy_uses_manual_seek_path_in_desktop_mode(monkeypatch):
    ...
    monkeypatch.setattr(plc, "is_motion_target_in_bounds", lambda *_args: True)
    ...
    monkeypatch.setattr(
        ...
        lambda x, y, speed, move_timeout, idle_timeout=20.0: (
            seek_calls.append((x, y, speed, move_timeout, idle_timeout)) or True
        ),
    )
    ...
    assert seek_calls == [(2100.0, 250.0, 123.0, 9.0, 4.0)]
...
def test_goto_xy_clears_move_type_when_xy_seek_times_out_without_motion(monkeypatch):
    ...
    monkeypatch.setattr(plc, "get_plc_io_mode", lambda: "server")
    monkeypatch.setattr(plc, "set_speed", lambda speed: True)
    monkeypatch.setattr(plc, "get_xy", lambda: (2000.0, float(GEOMETRY_CONFIG.y_min)))
    monkeypatch.setattr(plc.time, "sleep", lambda _s: None)
    monkeypatch.setattr(plc.time, "monotonic", lambda: next(monotonic_values))
    plc._TRUE_XY = [2000.0, float(GEOMETRY_CONFIG.y_min)]
    plc._LAST_X_DIR = 0
    plc._X_DEADZONE_LEFT = 0.0

    moved = plc.goto_xy(
        2100.0,
        float(GEOMETRY_CONFIG.y_max),
        move_timeout=1.0,
    )
...
def test_goto_xy_does_not_clear_move_type_when_xy_seek_times_out_after_motion(monkeypatch):
    ...
    monkeypatch.setattr(plc, "get_plc_io_mode", lambda: "server")
    monkeypatch.setattr(plc, "set_speed", lambda speed: True)
    monkeypatch.setattr(plc, "get_xy", lambda: (2000.5, float(GEOMETRY_CONFIG.y_min)))
    monkeypatch.setattr(plc.time, "sleep", lambda _s: None)
    monkeypatch.setattr(plc.time, "monotonic", lambda: next(monotonic_values))
    plc._TRUE_XY = [2000.0, float(GEOMETRY_CONFIG.y_min)]
    plc._LAST_X_DIR = 0
