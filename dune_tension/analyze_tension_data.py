...
def analyze_tension_data(apa_name, layer, color_by_zone=True):
    ...
    tension_summary_csv_path = f"data/tension_summaries/tension_summary_{apa_name}_{layer}.csv"
    ...
    latest_df = df_sorted.drop_duplicates(subset=['layer', 'side', 'wire_number'], keep='last')
    ...
    plt.subplot(1, 2, 2)
    sns.histplot(data=hist_df, x='tension', hue='side_label', element='step',
    ...
    with open(bad_wires_log_path, 'w') as f:
        for (layer_val, side), bad_wires in bad_wires_by_group.items():
            ...
            if bad_wires:
                ...
                f.write("  Bad wire_numbers (missing or no tension_pass=True): " + ', '.join(compressed) + "\n")
            else:
            ...
            if outliers:
                ...
                f.write("  Outlier wire_numbers (far from moving average): " + ', '.join(compressed_outliers) + "\n")
            else:
...
if __name__ == "__main__":
    ...
    for apa_name, layer in tasks:
        ...
        try:
            results = analyze_tension_data(apa_name, layer, color_by_zone=False)
