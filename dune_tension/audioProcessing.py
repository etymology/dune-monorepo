...
def save_audio_data(audio_data, file_name):
    """Save the recorded audio data to a compressed .npz file."""
    try:
        np.savez_compressed(file_name, audio_data=audio_data)
        # print(f"Audio data saved to {file_name}.npz")
    except Exception as e:
        print(f"An error occurred while saving audio data: {e}")
...
def get_pitch_autocorrelation(audio_data, samplerate, freq_low=20, freq_high=2000,show_plots=False):
    """
    ...
    if show_plots:
        ...
        plt.axvline(peak_lag / samplerate, color='r', linestyle='--', label=f'Dominant Frequency: {dominant_frequency:.2f} Hz')
        plt.xlabel("Lag [s]")
...
def get_pitch_naive_fft(audio_data: np.ndarray, samplerate: int, show_plots = False) -> tuple[float, float]:
...
def get_pitch_crepe(audio_data: np.ndarray, samplerate, model_capacity = 'tiny') -> tuple[float, float]:
    """Extract the pitch and confidence from the audio data using CREPE."""
    _, frequencies, confidence, _ = crepe.predict(audio_data, samplerate, model_capacity=model_capacity, viterbi=False,verbose=0)
