...
class TensionTestingApp:
    ...
    def handle_manual_record(self):
        ...
        if (audio_signal.size > 0):
            ...
            if frequency != 0.0:
                ...
                log_prompt = input(
                    f"Do you want to log the frequency? [wire number {curr_wirenum}](y/n): ")
                if log_prompt.lower() == 'y':
                    self.log_frequency_and_wire_number(
                        [frequency, confidence, curr_wirenum], "Output.csv")
    ...
    def handle_goto_next_wire(self):
        wire_number = str(
            int(self.config_manager.load_config()['current_wirenumber'])+1)
        self.goto_wire(wire_number)
