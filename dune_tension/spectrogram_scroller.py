...
class CrepePitchTracker:
    ...
    """
    def __init__(self, sampling_rate: int = 44100, step_ms: float = 10.0, buffer_sec: float = 3.0):
        global crepe
        if crepe is None:
...
class PestoPitchView:
    ...
    """
    def __init__(self, window_sec: float, ylim: tuple[float, float] | None, sr: int, step_len: int, conf_min: float = 0.1):
        self.window_sec = float(window_sec)
        self.sr = sr
        ...
        self.fig, self.ax = plt.subplots(figsize=(10, 5))
        self.scat = self.ax.scatter([], [], c=[], cmap="viridis", vmin=0.0, vmax=1.0, s=18)
        ...
        self.crepe_scat = self.ax.scatter([], [], marker="s", s=18, cmap="viridis", vmin=0.0, vmax=1.0)
        self.ax.set_xlabel("Time (s)")
    ...
    def add_points(self, samples: list[tuple[float, float, float]]):
        ...
        while self.times and self.times[0] < left:
            self.times.pop(0); self.pitches.pop(0); self.confs.pop(0)
    ...
    def add_crepe_points(self, samples):
        ...
        while self.crepe_times and self.crepe_times[0] < left:
            self.crepe_times.pop(0); self.crepe_pitches.pop(0); self.crepe_confs.pop(0)
    ...
    def add_ac_point(self, t_s, f0_hz):
        ...
        while self.ac_times and self.ac_times[0] < left:
            self.ac_times.pop(0); self.ac_pitches.pop(0)
    ...
    def update_plot(self):
        ...
        mask = np.isfinite(y)
        offsets = np.column_stack([t[mask], y[mask]]) if mask.any() else np.zeros((0,2))
        self.scat.set_offsets(offsets)
        self.scat.set_array(c[mask] if mask.any() else np.array([]))
        # Autocorr line data
        if len(self.ac_times) > 0:
            self.ac_line.set_data(np.array(self.ac_times, dtype=float), np.array(self.ac_pitches, dtype=float))
        else:
            self.ac_line.set_data([], [])
        # Keep fixed horizontal window
        xmax = t.max() if t.size else (self.ac_times[-1] if self.ac_times else self.window_sec)
        ...
        self.fig.suptitle(f"PESTO: {latest_pesto:.1f} Hz | Autocorr: {latest_ac:.1f} Hz")
        self.fig.canvas.draw_idle()
...
class App:
    def __init__(
        ...
        if self.mode == "spec":
            # Spectrogram view
            self.view = SpectrogramView(samplerate, fft_size, hop, window_sec, max_freq, db_range)
            ...
            # PESTO: tracker + view
            cfg = PestoConfig(step_ms=pesto_step_ms, model_name=pesto_model, sampling_rate=samplerate)
            self.pesto = PestoPitchTracker(cfg)
            self.view = PestoPitchView(window_sec, pesto_pitch_ylim, samplerate, self.pesto.step_len, conf_min=pesto_conf_min)
    ...
    def _spec_mag(self, frame: np.ndarray) -> np.ndarray:
        spec = np.fft.rfft(frame * self.win, n=self.fft_size)
        return np.abs(spec)[: getattr(self, "max_bin", len(spec)) ]
    ...
    def _profile_noise(self):
        ...
        self.ax.set_title("Profiling noise... keep quiet"); self.fig.canvas.draw_idle()
        ...
        if mags:
            M = np.vstack(mags)
            self.noise_mag = np.median(M, axis=0).astype(np.float32)
        else:
            self.noise_mag = None
        self.ax.set_title(title0); self.fig.canvas.draw_idle()
    ...
    def _autocorr_pitch(self, x: np.ndarray, fmin: float = 50.0, fmax: float = 1200.0) -> float | None:
    ...
    def _apply_noise_filter(self, mag: np.ndarray) -> np.ndarray:
        if getattr(self, "noise_filter_enabled", False) is False or self.noise_mag is None:
            return mag
    ...
    def process_chunk(self, chunk: np.ndarray):
        if self.mode == "spec":
            ...
            while self.sample_buf.size >= self.fft_size:
                ...
                # view computes own spectrum; noise subtraction only affects optional future extensions
            ...
            if results and self.ac_buf.size >= self.fft_size:
                ...
                # Spectrum & noise subtraction, then resynthesis
                spec = np.fft.rfft(frame * self.win, n=self.fft_size)
                mag = np.abs(spec)
                if self.noise_mag is not None:
                    max_bin = min(len(self.noise_mag), len(mag))
                    mag_clean = np.maximum(mag[:max_bin] - self.over_sub * self.noise_mag[:max_bin], 0.0)
                    scale = np.ones_like(mag)
                    scale[:max_bin] = mag_clean / (mag[:max_bin] + 1e-12)
                    spec_clean = spec * scale
                else:
                    spec_clean = spec
...
def parse_args():
    ap = argparse.ArgumentParser(description="Scrolling Spectrogram / PESTO Pitch Visualizer")
    ap.add_argument("--samplerate", type=int, default=44100, help="Sample rate (Hz)")
    ap.add_argument("--fft", type=int, default=2048, help="FFT size (power of 2 recommended)")
    ap.add_argument("--hop", type=int, default=512, help="Hop size (samples)")
    ap.add_argument("--window", type=float, default=10.0, help="Visible window (seconds)")
    ap.add_argument("--max-freq", type=float, default=8000.0, help="Max frequency shown (Hz) [spectrogram]")
    ap.add_argument("--db-range", type=float, default=80.0, help="Dynamic range (dB) [spectrogram]")
    ap.add_argument("--device", type=str, default=None, help="Input device name or index")
    ap.add_argument("--demo", action="store_true", help="Run in demo mode (no microphone)")

    # Spectrogram noise filter
    ap.add_argument("--noise-sec", type=float, default=2.0, help="Seconds to record for noise profiling at startup")
    ap.add_argument("--over-sub", type=float, default=1.0, help="Noise over-subtraction factor (1.0 = equal)")
    ap.add_argument("--no-noise-filter", action="store_true", help="Disable noise filter (no profiling/subtraction)")

    # PESTO
    ap.add_argument("--pesto", action="store_true", help="Enable PESTO pitch mode (requires pesto-pitch, torch, torchaudio)")
    ap.add_argument("--pesto-step-ms", type=float, default=10.0, help="PESTO step size in ms (streaming)")
    ap.add_argument("--pesto-model", type=str, default="mir-1k_g7", help="PESTO pretrained model name or path")
    ap.add_argument("--crepe", action="store_true", help="Overlay CREPE pitch estimation alongside PESTO")
    ap.add_argument("--crepe-step-ms", type=float, default=10.0, help="CREPE step size in ms")
    ap.add_argument("--pesto-min", type=float, default=50.0, help="Suggested Y-axis min pitch (Hz)")
    ap.add_argument("--pesto-max", type=float, default=2000.0, help="Suggested Y-axis max pitch (Hz)")
    ap.add_argument("--pesto-conf-min", type=float, default=0.1, help="Minimum PESTO confidence to plot a point (0..1)")
...
def main():
    ...
    if args.demo or sd is None:
        ...
        try:
            ...
            print("Falling back to demo mode. Use --device to select input or install sounddevice.")
            source = DemoSource(args.samplerate, args.hop)
