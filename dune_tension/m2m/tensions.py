...
def ExtractTensions(csvFile, apaLayer):
    ...
    if csvFile[-4:] != ".csv":
        sys.exit(
            " ExtractTensions() - ERROR: the specified input file is not a '.csv' type! \n"
        )

    if apaLayer not in apaLayers:
        sys.exit(
            " ExtractTensions() - ERROR: invalid APA layer specified ... please use 'X','U', 'V' or 'G'! \n"
        )
    ...
    print(
        f" ExtractTensions() - INFO: successfully extracted {len(list_sideA)} tensions for side A and {len(list_sideB)} for side B"
    )
