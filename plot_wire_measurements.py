...
cursor.execute(
    "SELECT frequency, confidence, tension FROM tension_data WHERE wire_number='1000'"
)
td_rows = cursor.fetchall()

cursor.execute(
    "SELECT frequency, confidence, tension FROM tension_samples WHERE wire_number='1000'"
)
...
ax1.set_title(
    "Histogram of Frequency Measurements for Wire 1000", fontsize=14, fontweight="bold"
)
...
ax1.axvline(
    mean_f, color="red", linestyle="--", linewidth=2, label=f"Mean: {mean_f:.2f} Hz"
)
ax1.axvline(
    median_f,
    color="green",
