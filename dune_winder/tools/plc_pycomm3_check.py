...
def run_check(
    ...
    *,
    ...
    with LogixDriver(
        ...
        tags_to_verify = list(
            dict.fromkeys(read_tags + [request.tag for request in write_requests])
        )
        if tags_to_verify:
        ...
        if restore:
            for request in write_requests:
                restore_result = driver.write(
                    (request.tag, original_values[request.tag])
                )
                if isinstance(restore_result, list):
                    normalized = [
                        _normalize_write_result(item) for item in restore_result
                    ]
                else:
