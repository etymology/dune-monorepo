...
def _available_distance(
    value: float, low: float, high: float, direction: int, margin: float
) -> float:
    if direction < 0:
...
def _build_line_case(
    ...
    for x_dir in x_dirs:
        for y_dir in y_dirs:
            x_room = (
                _available_distance(
                    start_x, limits.limit_left, limits.limit_right, x_dir, 25.0
                )
                / 2.0
            )
            y_room = _available_distance(
                start_y, limits.limit_bottom, limits.limit_top, y_dir, 25.0
            )
    ...
    raise RuntimeError(
        "Could not build a safe line_pair queue-cap test case from the current position"
    )
...
def _build_circle_case(
    ...
    raise RuntimeError(
        "Could not build a safe circle_single queue-cap test case from the current position"
    )
...
def _run_case(
    ...
    *,
    ...
    seqs = [
        int(queue._read_one(f"SegQueue[{i}].Seq")) for i in range(len(case.segments))
    ]
    xy = [
        (
    ...
    *,
    ...
    with MotionQueueClient(plc_path) as motion:
        ...
        if (
            ...
            raise RuntimeError(
                "Refusing to run because the PLC queue is not idle and fault-free"
            )
